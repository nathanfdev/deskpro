...
DeskPRO.Form.FormValidator = new Class({
	...
	runCallback: function(field, errors) {
		...
		var el = $('#' + field.getId());
		var el_id = field.getId();
		var error_container = el.closest(this.options['errorContainerSelector']);
		
		// Hide all LI's first to 'reset'
		// ul .my_element_id
		$(this.options['errorListSelector'] + ' .' + el_id, error_container).addClass(this.options['errorClassHide']);
		...
		if (errors && errors.length) {
			for (var i = 0; i < errors.length; i++) {
				...
				$(this.options['errorListSelector'] + ' .' + el_id + '.' + errors[i], error_container).removeClass(this.options['errorClassHide']);
			}
		}
