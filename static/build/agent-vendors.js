...
if(G=="relative"&&g.ratio<=1){x=Math.min((g[f.axis]-k[f.axis]),Math.max(0,x))}if(!x){x=0}if(k[f.axis]>=g[f.axis]){x=0}C()
};this.tinyscrollbar_scrolltop=function(G){b(G)};function C(){v.obj.css(r,x/c.ratio);g.obj.css(r,-x);u.start=v.obj.offset()[r];
var G=B.toLowerCase();c.obj.css(G,q[f.axis]);q.obj.css(G,q[f.axis]);v.obj.css(G,v[f.axis])}function y(){v.obj.bind("mousedown",j);
