...
m.addClass("with-scrollbar");return m.tinyscrollbar(f)}if(z.is(".scroll-setup")){return z}z.addClass("scroll-setup");var k={obj:a(".scroll-viewport:first",this)};
var g={obj:a(".scroll-content:first",this)};var c={obj:a(".scrollbar:first",this)};var q={obj:a(".track:first",c.obj)};var v={obj:a(".thumb:first",c.obj)};
var l=f.axis=="x",r=l?"left":"top",B=l?"Width":"Height";var x,F={start:0,now:0},u={};var d=null;var E=null;if(this.length>1){this.each(function(){a(this).tinyscrollbar(f)
});return this}this.initialize=function(){this.tinyscrollbar_update();y()};this.tinyscrollbar_update=function(G){if(!G){G="relative"
}k[f.axis]=k.obj[0]["offset"+B];g[f.axis]=g.obj[0]["scroll"+B];g.ratio=k[f.axis]/g[f.axis];c.obj.toggleClass("disable",g.ratio>=1);
q[f.axis]=f.size=="auto"?k[f.axis]:f.size;v[f.axis]=Math.min(q[f.axis],Math.max(0,(f.sizethumb=="auto"?(q[f.axis]*g.ratio):f.sizethumb)));
if(v[f.axis]<30){v[f.axis]=30;f.sizethumb=30}c.ratio=f.sizethumb=="auto"?(g[f.axis]/q[f.axis]):(g[f.axis]-k[f.axis])/(q[f.axis]-v[f.axis]);
if(G=="relative"&&g.ratio<=1){x=Math.min((g[f.axis]-k[f.axis]),Math.max(0,x))}if(!x){x=0}C()};this.tinyscrollbar_scrolltop=function(G){b(G)
};function C(){v.obj.css(r,x/c.ratio);g.obj.css(r,-x);u.start=v.obj.offset()[r];var G=B.toLowerCase();c.obj.css(G,q[f.axis]);
q.obj.css(G,q[f.axis]);v.obj.css(G,v[f.axis]);g.obj.css(r,-x)}function y(){v.obj.bind("mousedown",j);q.obj.bind("mouseup",A);
...
})})(jQuery);(function(a){a.fn.TextAreaExpander=function(e,d){var c=!(a.browser.msie||a.browser.opera);function b(j){j=j.target||j;
var k=j.value.length,g=j.offsetWidth;if(k!=j.valLength||g!=j.boxWidth){if(c&&(k<j.valLength||g!=j.boxWidth)){j.style.height="0px"
}var f=Math.max(j.expandMin,Math.min(j.scrollHeight,j.expandMax));j.style.overflow=(j.scrollHeight>f?"auto":"hidden");j.style.height=f+"px";
j.valLength=k;j.boxWidth=g}return true}this.each(function(){if(this.nodeName.toLowerCase()!="textarea"){return}var f=this.className.match(/expand(\d+)\-*(\d+)*/i);
this.expandMin=e||(f?parseInt("0"+f[1],10):0);this.expandMax=d||(f?parseInt("0"+f[2],10):99999);b(this);if(!this.Initialized){this.Initialized=true;
a(this).css("padding-top",0).css("padding-bottom",0);a(this).bind("keyup",b).bind("focus",b)}});return this}})(jQuery);(function(b){var a=function(c){if(!c){var c=window.event
}c.cancelBubble=true;if(c.stopPropagation){c.stopPropagation()}};b.fn.checkbox=function(j){try{document.execCommand("BackgroundImageCache",false,true)
}catch(k){}var d={cls:"jquery-checkbox",empty:"empty.png"};d=b.extend(d,j||{});var c=function(f){var e=f.checked;var l=f.disabled;
var g=b(f);if(f.stateInterval){clearInterval(f.stateInterval)}f.stateInterval=setInterval(function(){if(f.disabled!=l){g.trigger((l=!!f.disabled)?"disable":"enable")
}if(f.checked!=e){g.trigger((e=!!f.checked)?"check":"uncheck")}},10);return g};return this.each(function(){var f=this;var e=c(f);
if(f.wrapper){f.wrapper.remove()}f.wrapper=b('<span class="'+d.cls+'"><span class="mark"><img src="'+d.empty+'" /></span></span>');
f.wrapperInner=f.wrapper.children("span:eq(0)");f.wrapper.hover(function(l){f.wrapperInner.addClass(d.cls+"-hover");a(l)},function(l){f.wrapperInner.removeClass(d.cls+"-hover");
a(l)});e.css({position:"absolute",zIndex:-1,visibility:"hidden"}).after(f.wrapper);var g=false;if(e.attr("id")){g=b("label[for="+e.attr("id")+"]");
if(!g.length){g=false}}if(!g){g=e.closest?e.closest("label"):e.parents("label:eq(0)");if(!g.length){g=false}}if(g){g.hover(function(l){f.wrapper.trigger("mouseover",[l])
},function(l){f.wrapper.trigger("mouseout",[l])});g.click(function(l){e.trigger("click",[l]);a(l);return false})}f.wrapper.click(function(l){e.trigger("click",[l]);
a(l);return false});e.click(function(l){a(l)});e.bind("disable",function(){f.wrapperInner.addClass(d.cls+"-disabled")}).bind("enable",function(){f.wrapperInner.removeClass(d.cls+"-disabled")
});e.bind("check",function(){f.wrapper.addClass(d.cls+"-checked")}).bind("uncheck",function(){f.wrapper.removeClass(d.cls+"-checked")
});b("img",f.wrapper).bind("dragstart",function(){return false}).bind("mousedown",function(){return false});if(window.getSelection){f.wrapper.css("MozUserSelect","none")
}if(f.checked){f.wrapper.addClass(d.cls+"-checked")}if(f.disabled){f.wrapperInner.addClass(d.cls+"-disabled")}})}})(jQuery);
(function(a){a.fn.tokenField=function(q){var e={regex:/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}$/i,delimiters:", ",max:0,nested:false,badToken:function(){a(this).val("")
},tooMany:function(){a(this).val("")}};if(q){a.extend(e,q)}function m(u){try{console.log(u)}catch(r){}}function f(r,u){return a.map(u,function(x){return g(r,x)
}).join("")}function p(r){return"<div class='token-input'><input type='text' size='1'/><span class='token-input-sizer'>###</span></div>"
}function c(r){return String.fromCharCode(r).match(RegExp("["+e.delimiters+"]"))}function d(r){return r.match(e.regex)}function l(r){r.click(function(){a(".token-input",this).siblings(".selected-token").removeClass("selected-token");
a(".token-input input",this).focus();return false});a(".token-input input",r).keydown(function(u){if(u.which==9&&a(this).val()){a(this).blur();
a(this).focus();return false}return true}).keypress(function(u){a(this).next(".token-input-sizer").html(a(this).val()+"###");
if(u.which==13||c(u.which)){a(this).blur();if(c(u.which)){a(this).focus()}return false}return true}).keyup(function(){a(this).next(".token-input-sizer").html(a(this).val()+"###");
return true}).focus(function(){a(this).next(".token-input-sizer").html(a(this).val()+"###");return true}).blur(function(x){if(!a(this).data("blur")){a(this).data("blur",true);
if(e.max==0||a(this).closest(".token-input").siblings(".token").length<e.max){if(d(a(this).val())){b(a(g(a(this).attr("name"),a(this).val())).insertBefore(a(this).closest(".token-input")));
if(!e.nested){var v=a(this).closest(".token-field").find("input:hidden");var u=v.val().split(",");u.splice(0,0,a(this).val());
v.val(u.join(","))}a(this).val("")}else{if(e.badToken){this.badToken=e.badToken;this.badToken()}}}else{if(e.tooMany){this.tooMany=e.tooMany;
this.tooMany()}}a(this).removeData("blur")}return true});b(a(".token",r))}function b(r){r.click(function(){a(this).focus();
return false}).keydown(function(u){if(a(this).hasClass("selected-token")){if(u.which==8){k(a(this));return false}}return true
}).blur(function(){a(this).removeClass("selected-token")}).focus(function(){a(this).siblings(".selected-token").removeClass("selected-token");
a(this).addClass("selected-token")});a(".token-x",r).click(function(){a(this).closest(".token").siblings(".selected-token").removeClass("selected-token");
k(a(this).closest(".token"));return false})}function j(r){r=r.replace(/^\s*(.+)\s*$/,"$1");if(!r){return[]}return a.map(r.split(","),function(u){if(d(u)){return u
}m("Warning: ignoring bad token - "+u);return null})}function k(x){if(!e.nested){var v=x.siblings(".token").andSelf().index(x);
var u=x.closest(".token-field").find("input:hidden");var r=u.val().split(",");r.splice(v,1);u.val(r.join(","))}x.remove()
}function g(r,u){return"<a href='#' class='token'><span><span><span><span>"+u+(e.nested?"<input type='hidden' value='"+u+"' name='"+r+"'/>":"")+"<span href='#' class='token-x'>x</span></span></span></span></span></a>"
}return this.each(function(){var z=a(this).attr("id");var u=a(this).attr("name");var r=a(this).attr("class");var v="";var y=j(a(this).val());
if(a(this).height()>0){v=v+"min-height: "+a(this).height()+"px; height: auto !important; height: "+a(this).height()+"px;"
}if(a(this).width()>0){v=v+"width: "+a(this).width()+"px;"}if(e.max>0&&y.length>e.max){m("Warning: ignoring extra tokens after maximum of "+e.max);
y=y.slice(0,e.max)}var x=a("<div></div>").attr({"class":r,id:z,style:v}).addClass("token-field").append(f(u,y)).append(p(u)).append("<div style='clear:both'></div>").replaceAll(this);
if(!e.nested){x.prepend("<input type='hidden' value='"+y.join(",")+"' name='"+u+"'/>")}l(x)})}})(jQuery);(function(g){var l,m,j=[],p=window;
g.fn.tinymce=function(q){var a=this,v,f,u,d,r,e="",c="";if(!a.length){return a}if(!q){return tinyMCE.get(a[0].id)}a.css("visibility","hidden");
function b(){var x=[],y=0;if(k){k();k=null}a.each(function(C,B){var D,z=B.id,A=q.oninit;if(!z){B.id=z=tinymce.DOM.uniqueId()
}D=new tinymce.Editor(z,q);x.push(D);D.onInit.add(function(){var E,F=A;a.css("visibility","");if(A){if(++y==x.length){if(tinymce.is(F,"string")){E=(F.indexOf(".")===-1)?null:tinymce.resolve(F.replace(/\.\w+$/,""));
F=tinymce.resolve(F)}F.apply(E||tinymce,x)}}})});g.each(x,function(z,A){A.render()})}if(!p.tinymce&&!m&&(v=q.script_url)){m=1;
u=v.substring(0,v.lastIndexOf("/"));if(/_(src|dev)\.js/g.test(v)){c="_src"}d=v.lastIndexOf("?");if(d!=-1){e=v.substring(d+1)
}p.tinyMCEPreInit=p.tinyMCEPreInit||{base:u,suffix:c,query:e};if(v.indexOf("gzip")!=-1){r=q.language||"en";v=v+(/\?/.test(v)?"&":"?")+"js=true&core=true&suffix="+escape(c)+"&themes="+escape(q.theme)+"&plugins="+escape(q.plugins)+"&languages="+r;
if(!p.tinyMCE_GZ){tinyMCE_GZ={start:function(){tinymce.suffix=c;function x(y){tinymce.ScriptLoader.markDone(tinyMCE.baseURI.toAbsolute(y))
}x("langs/"+r+".js");x("themes/"+q.theme+"/editor_template"+c+".js");x("themes/"+q.theme+"/langs/"+r+".js");g.each(q.plugins.split(","),function(y,z){if(z){x("plugins/"+z+"/editor_plugin"+c+".js");
x("plugins/"+z+"/langs/"+r+".js")}})},end:function(){}}}}g.ajax({type:"GET",url:v,dataType:"script",cache:true,success:function(){tinymce.dom.Event.domLoaded=1;
m=2;if(q.script_loaded){q.script_loaded()}b();g.each(j,function(y,x){x()})}})}else{if(m===1){j.push(b)}else{b()}}return a
};g.extend(g.expr[":"],{tinymce:function(a){return a.id&&!!tinyMCE.get(a.id)}});function k(){function c(f){if(f==="remove"){this.each(function(u,r){var q=d(r);
if(q){q.remove()}})}this.find("span.mceEditor,div.mceEditor").each(function(u,r){var q=tinyMCE.get(r.id.replace(/_parent$/,""));
if(q){q.remove()}})}function a(r){var f=this,q;if(r!==l){c.call(f);f.each(function(v,u){var x;if(x=tinyMCE.get(u.id)){x.setContent(r)
}})}else{if(f.length>0){if(q=tinyMCE.get(f[0].id)){return q.getContent()}}}}function d(f){var q=null;(f)&&(f.id)&&(p.tinymce)&&(q=tinyMCE.get(f.id));
return q}function e(f){return !!((f)&&(f.length)&&(p.tinymce)&&(f.is(":tinymce")))}var b={};g.each(["text","html","val"],function(u,q){var r=b[q]=g.fn[q],f=(q==="text");
g.fn[q]=function(v){var z=this;if(!e(z)){return r.apply(z,arguments)}if(v!==l){a.call(z.filter(":tinymce"),v);r.apply(z.not(":tinymce"),arguments);
return z}else{var x="";var y=arguments;(f?z:z.eq(0)).each(function(B,A){var C=d(A);x+=C?(f?C.getContent().replace(/<(?:"[^"]*"|'[^']*'|[^'">])*>/g,""):C.getContent()):r.apply(g(A),y)
});return x}}});g.each(["append","prepend"],function(u,f){var r=b[f]=g.fn[f],q=(f==="prepend");g.fn[f]=function(v){var x=this;
if(!e(x)){return r.apply(x,arguments)}if(v!==l){x.filter(":tinymce").each(function(z,y){var A=d(y);A&&A.setContent(q?v+A.getContent():A.getContent()+v)
});r.apply(x.not(":tinymce"),arguments);return x}}});g.each(["remove","replaceWith","replaceAll","empty"],function(f,q){var r=b[q]=g.fn[q];
g.fn[q]=function(){c.call(this,q);return r.apply(this,arguments)}});b.attr=g.fn.attr;g.fn.attr=function(y,u,x){var f=this;
if((!y)||(y!=="value")||(!e(f))){return b.attr.call(f,y,u,x)}if(u!==l){a.call(f.filter(":tinymce"),u);b.attr.call(f.not(":tinymce"),y,u,x);
return f}else{var v=f[0],r=d(v);return r?r.getContent():b.attr.call(g(v),y,u,x)}}}})(jQuery);(function(a9,aK){var aR="none",aq="LoadedContent",a8=false,aP="resize.",aW="y",aU="auto",a6=true,ar="nofollow",aY="x";
...
};d=false;return this};String.prototype.trim=function(){return this.replace(/^\s+|\s+$/g,"")}})(jQuery);document.createElement("canvas").getContext||function(){function L(){}function O(b){this.type_=b,this.r1_=this.y1_=this.x1_=this.r0_=this.y0_=this.x0_=0,this.colors_=[]
...
}();(function(){function r(d,c){d&&(this.element=d,j.remove(d),j.removeDetached(),this.options=m.extend({radii:[5,10],color:"#000",dashWidth:1.8,dashes:12,opacity:1,padding:3,speed:0.7,build:!0},c||{}),this._position=0,this._state="stopped",this.build(),j.add(this))
}function u(b){this.element=b}function v(b){return b*Math.PI/180}var q={Version:"2.0_b1"},p=Array.prototype.slice,m={extend:function(e,d){for(var f in d){e[f]=d[f]
}return e},"break":{},_each:function(f,e){for(var x=0,g=f.length;x<g;x++){e(f[x])}},each:function(g,f,z){var y=0;try{this._each(g,function(b){f.call(z,b,y++)
})}catch(x){if(x!=m["break"]){throw x}}},bind:function(e,d){var f=p.call(arguments,2);return function(){return e.apply(d,f.concat(p.call(arguments)))
}},scroll:function(e,d){if(!d){return e}var f=e.slice(0,d);return e.slice(d,e.length).concat(f)},any:function(f,e,x){var g=!1;
m.each(f||[],function(b,c){if(g|=e.call(x,b,c)){return m["break"]}});return !!g},member:function(e,d){var f=!1;m.any(e||[],function(b){if(f=b===d){return !0
}});return f},select:function(f,e,x){var g=[];m.each(f||[],function(b,c){e.call(x,b,c)&&(g[g.length]=b)});return g},without:function(d){var c=p.call(arguments,1);
return m.select(d,function(b){return !m.member(c,b)})}},l={drawRoundedRectangle:function(a,B){var A=m.extend({top:0,left:0,width:0,height:0,radius:0},B||{}),z=A.left,y=A.top,x=A.width,f=A.height,A=A.radius;
a.beginPath(),a.moveTo(z+A,y),a.arc(z+x-A,y+A,A,v(-90),v(0),!1),a.arc(z+x-A,y+f-A,A,v(0),v(90),!1),a.arc(z+A,y+f-A,A,v(90),v(180),!1),a.arc(z+A,y+A,A,v(-180),v(-90),!1),a.closePath(),a.fill()
}},k=function(){function e(b){var g=[];b.indexOf("#")==0&&(b=b.substring(1)),b=b.toLowerCase();if(b.replace(d,"")!=""){return null
}b.length==3?(g[0]=b.charAt(0)+b.charAt(0),g[1]=b.charAt(1)+b.charAt(1),g[2]=b.charAt(2)+b.charAt(2)):(g[0]=b.substring(0,2),g[1]=b.substring(2,4),g[2]=b.substring(4));
for(b=0;b<g.length;b++){g[b]=parseInt(g[b],16)}g.red=g[0],g.green=g[1],g.blue=g[2];return g}var d=RegExp("[0123456789abcdef]","g"),f=function(){function b(x,g,y){x=x.toString(y||10);
return Array(g-x.length).join("0")+x}return function(a,x,g){return"#"+b(a,2,16)+b(x,2,16)+b(g,2,16)}}();return{hex2rgb:e,hex2fill:function(a,y){typeof y=="undefined"&&(y=1);
var x=y,g=e(a);g[3]=x,g.opacity=x;return"rgba("+g.join()+")"},rgb2hex:f}}();m.extend(q,{enabled:!1,support:{canvas:function(){var b=document.createElement("canvas");
return !!b.getContext&&!!b.getContext("2d")}()},dom:function(){var d,c;d=c=function(){throw"Using Spinners with a CSS Selector requires a selector engine, include one of: Sizzle (jQuery 1.4+/Prototype 1.7+), NWMatcher or Slick (MooTools 1.3+)."
},window.Sizzle?(d=Sizzle,c=Sizzle.matches):window.jQuery?(d=jQuery.find,c=function(f,e){return jQuery(f).is(e)}):window.NWMatcher&&NW.Dom?(d=NW.Dom.select,c=NW.Dom.match):window.Prototype&&Prototype.Selector?(d=Prototype.Selector.select,c=Prototype.Selector.match):window.Slick&&(d=function(f,e){return Slick.search(e||document,f)
},c=Slick.match);return{select:d,match:c}}(),init:function(){if(!this.support.canvas&&!window.G_vmlCanvasManager){if(window.attachEvent&&navigator.userAgent.indexOf("Opera")===-1){alert("Spinners requires ExplorerCanvas (excanvas.js)")
}else{return}}window.G_vmlCanvasManager&&window.G_vmlCanvasManager.init_(document),this.enabled=!0},create:function(b,d){u.create(b,d);
return this.get(b)},get:function(b){return new u(b)},play:function(b){this.get(b).play();return this},pause:function(b){this.get(b).pause();
return this},toggle:function(b){this.get(b).toggle();return this},stop:function(b){this.get(b).stop();return this},remove:function(b){this.get(b).remove();
return this},removeDetached:function(){j.removeDetached();return this},getDimensions:function(b){b=j.get(b)[0].getLayout().workspace.radius*2;
return{width:b,height:b}}});var j={spinners:[],get:function(d){if(d){var c=[];m.each(this.spinners,function(a){a&&(d.nodeType==1?a.element==d:q.dom.match(a.element,d))&&c.push(a)
});return c}},add:function(b){this.spinners.push(b)},remove:function(d){var c=[];m.each(this.spinners,function(a){(d.nodeType==1?a.element==d:q.dom.match(a.element,d))&&c.push(a.element)
}),m.each(c,m.bind(function(b){this.removeByElement(b)},this))},removeByElement:function(b){if(b=this.get(b)[0]){b.remove(),this.spinners=m.without(this.spinners,b)
}},removeDetached:function(){return function(){m.each(this.spinners,function(b){b&&!b.isAttached()&&this.remove(b.element)
},this)}}()};m.extend(u,{create:function(d,c){if(d){var x=c||{},f=[];d.nodeType==1?f.push(new r(d,x)):m.each(q.dom.select(d)||[],function(b){f.push(new r(b,x))
});return f}}}),m.extend(u.prototype,{items:function(){return j.get(this.element)},play:function(){m.each(this.items(),function(b){b.play()
});return this},stop:function(){m.each(this.items(),function(b){b.stop()});return this},pause:function(){m.each(this.items(),function(b){b.pause()
});return this},toggle:function(){m.each(this.items(),function(b){b.toggle()});return this},remove:function(){j.remove(this.element);
return this}}),m.extend(r.prototype,function(){function a(){var b=this.options.speed*1000/this.options.dashes;this.nextPosition(),this._playTimer=window.setTimeout(m.bind(a,this),b)
}return{remove:function(){this.canvas&&(this.stop(),this.canvas.parentNode.removeChild(this.canvas),this.ctx=this.canvas=null)
},build:function(){this.remove();var d=this.getLayout().workspace.radius,c=d*2;this.canvas=document.createElement("canvas"),this.canvas.style.zoom=1,this.canvas.height=c,this.canvas.width=c,document.body.appendChild(this.canvas),window.G_vmlCanvasManager&&G_vmlCanvasManager.initElement(this.canvas),this.ctx=this.canvas.getContext("2d"),this.ctx.globalAlpha=this.options.opacity,this.element.appendChild(this.canvas),this.ctx.translate(d,d),this.drawPosition(0);
return this},getLayout:function(){if(this._layout){return this._layout}var F=this.options,E=F.dashes,D=F.radii,C=F.dashWidth,B=Math.min(D[0],D[1]),D=Math.max(D[0],D[1]),A=Math.ceil(Math.max(Math.max(C,D),Math.sqrt(D*D+C/2*(C/2))));
A+=F.padding;for(var F=A,z=1/E,y=[],x=0;x<E;x++){y.push((x+1)*z)}return this._layout=E={workspace:{radius:F,opacities:y},dash:{position:{top:A-D,left:A-C/2},dimensions:{width:C,height:D-B}}}
},isAttached:function(){if(!this.element){return !1}var b;for(b=this.element;b&&b.parentNode;){b=b.parentNode}return !!b&&!!b.body
...
},drawPosition:function(f){var A=this.getLayout().workspace,z=A.radius*2,y=-1*A.radius,x=this.options.dashes;this.ctx.clearRect(y,y,z,z),z=v(360/x),f=m.scroll(A.opacities,f*-1);
for(A=0;A<x;A++){this.drawDash(f[A],this.options.color),this.ctx.rotate(z)}},drawDash:function(g,f){this.ctx.fillStyle=k.hex2fill(f,g);
var z=this.getLayout(),y=z.workspace.radius,x=z.dash.position,z=z.dash.dimensions;l.drawRoundedRectangle(this.ctx,{top:x.top-y,left:x.left-y,width:z.width,height:z.height,radius:Math.min(z.height,z.width)/2})
},play:function(){if(this._state!="playing"){this._state="playing";var b=this.options.speed*1000/this.options.dashes;this._playTimer=window.setTimeout(m.bind(a,this),b);
...
eval(function(j,b,l,d,g,f){g=function(a){return(a<b?"":g(parseInt(a/b)))+((a=a%b)>35?String.fromCharCode(a+29):a.toString(36))
