...
if(d=="-"||d=="rem"||d=="del"||d=="sub"){c-=a;if(c<0){c=0}}else{c+=a}b.text(c)},getPlainTpl:function(b){var b=$(b);var a=b.get(0).innerHTML;
a=a.replace(/%startScript%/g,"<script>");a=a.replace(/%endScript%/g,"<\/script>");return a},showSavePuff:function(a){var b=$('<div class="load-puff" style="display: none; opacity: 0" />');
...
})},225)})},slugify:function(a){a=a.replace(/[^a-zA-Z0-9\-]/g,"-");a=a.replace(/\-{2,}/g,"-");a=a.replace(/^\-/,"");a=a.replace(/\-$/,"");
return a}}},initPage:function(){this._initBasic();this._initSections();this._initRoutes();this._initWindowInterface();this._initLayout();
this._initInterfaceServices();$("#page_loading").remove();$("#loading_css").remove();this.fragmentRouter=window.DeskPRO_FragmentRouter;
this.fragmentRouter.setBaseUrl(BASE_URL);var a=this;$.history.init(function(b){a.loadHashPath(b)},{unescape:",/:"});Orb.Class_GC_Start(5000);
Orb.Class_GC_Callbacks.push(function(b,c){a.messageBroker.removeTaggedEvents(c)})},loadHashPath:function(c){if(this.DEBUG.disableUrlFragments){return
...
a._promptOverlay_callback_no($("#prompt_overlay_input").val(),$("#prompt_overlay"));a._promptOverlay_callback_no=function(){};
$("#prompt_overlay_input").val("")}).bind(this));$(".okay-trigger",b.wrapperEl).click((function(){b.overlay.closeOverlay();
a._promptOverlay_callback_yes($("#prompt_overlay_input").val(),$("#prompt_overlay"));a._promptOverlay_callback_yes=function(){};
$("#prompt_overlay_input").val("")}).bind(this))}})},showStatusMessage:function(c,a){a=Object.merge({btnCallback:null,btnText:"Dismiss",autoClose:4500,extraClasses:""},a||{});
...
}else{if(a(".OpenChats")){b=this.sections.chat_section}else{if(a(".Idea")){b=this.sections.ideas_section}else{if(a(".TicketFilter")||a(".RecycleBin")){b=this.sections.tickets_section
}}}}}}}if(!b&&this.DEBUG.useTestSection){b=this.sections.test_section}if(!b){console.error("List page fragment has no section: %s: %o",c.getMetaData("fragmentClass",""),c);
return}b.setListPageFragment(c);this.listPage=c;this.updateWindowUrlFragment()},getListPage:function(){return this.listPage
},addPageTab:function(a){this.pageTabStrip.addTab(a)},removePage:function(b){var a=this.pageTabStrip.findTabByPage(b);if(a){this.pageTabStrip.removeTabById(a)
}},addPageRouteLoader:function(a,b){if(this.routePrefixes[a]==undefined){this.routePrefixes[a]=[]}this.routePrefixes[a].push(b)
},runPageRoute:function(a,b){var d=false;var c=this.parseRoute(a);if(b){c=Object.merge(b,c)}Object.each(this.routePrefixes,function(e,f){if(a.indexOf(f)==0){Array.each(e,function(g){g(c);
d=true});if(c.stopListeners){return true}}},this);if(!d){console.warn("Unknown route: %s",a)}},parseRoute:function(a){var g=a.split(":");
var d=g.shift();var f=null;if(d.indexOf(".")!=-1){var c=d.split(".");d=c.shift();f=c.pop()}var b=g.pop();var e={route:a,master:d,masterTag:f,sections:g,url:b,stopListeners:false};
return e},runPageRouteFromElement:function(c){c=$(c);if(c.is(".cancel-route")){return}if(!c.data("route")){console.warn("Element has no route: %o",c);
return}var b={};if(c.data("route-title")){b.title=c.data("route-title");if(b.title=="@text"){b.title=c.text().trim().replace(/[\n\r]/g," ").replace(/\s+/g," ")
}else{if(b.title=="@title"){b.title=c.attr("title")}else{if(b.title.test(/^@selector\((.*?)\)$/)){var d=b.title.match(/^@selector\((.*?)\)$/)[1];
var a=null;if(d[0]=="#"){a=$(d)}else{a=$(d,c)}if(a&&a.length){b.title=a.text().trim().replace(/[\n\r]/g," ").replace(/\s+/g," ")
}else{delete b.title}}}}}this.runPageRoute(c.data("route"),b)},loadRoute:function(a){a.openInSection=a.master;switch(a.openInSection){case"listpane":this.loadListPane(a.url,a);
break;default:this.loadPage(a.url,a);break}},loadListPane:function(a,b,c){$("#dp_list > section").removeClass("on");$("#dp_list_loading").addClass("on");
...
d.setMetaData("routeUrl",a);if(b){d.setMetaData("routeData",b)}this.setListPage(d);if(c){c(d)}}).bind(this))},loadPage:function(b,c,d){if(!c||(!c.ignoreExist)){var a=this.pageTabStrip.getTabByRouteUrl(b);
if(a&&!(a.page.allowDupe&&a.page.TYPENAME!="loading")){this.pageTabStrip.removeTabById(a.id);return}}c.tabPlaceholderId=this.pageTabStrip.addTabPlaceholder(b,c);
this._doAjaxLoadRoute(b,c,(function(f){var e=this.createPageFragment(f);e.setMetaData("routeUrl",b);if(c){e.setMetaData("routeData",c);
if(c.tabPlaceholderId){e.setMetaData("tabPlaceholderId",c.tabPlaceholderId)}}if(c.fragment){e.setMetaData("fragment",c.fragment)
}this.addPageTab(e);if(d){d(e)}}).bind(this))},_doAjaxLoadRoute:function(c,d,a){d=d||{};if(!c){console.error("No URL provided! routeData: %o",d);
...
d.xhr=e}},createPageFragment:function(html,classname,force_classname){pageMeta={title:false,fragmentClass:classname||"DeskPRO.Agent.PageFragment.Basic"};
...
var matches=regex.exec(html)}if(matches&&matches.length){eval(matches[1]);html=html.replace(matches[0],"")}if(force_classname){pageMeta.fragmentClass=classname
...
}if(typeof e=="string"){e=[e]}var d=this.volume;if(options.volume){d=options.volume}d=d+0;var b=[];b.push("<audio ");if(d!=1){b.push(' volume="'+d+'" ')
}if(options.loop){b.push(' loop="loop" ')}b.push(">");Array.each(e,function(g){b.push('<source src="'+g+'" />')});b.push("</audio>");
b=b.join("");var c=$(b);c.get(0).volume=d;if(options.destroyAfter){c.bind("ended",function(){$(this).remove()})}if(options.appendTo){$(options.appendTo).append(c)
}else{c.appendTo("body")}if(options.autoplay){c.get(0).play()}return c},playLibrarySound:function(b,a){var c=[ASSETS_BASE_URL+"/sounds/"+b+".mp3",ASSETS_BASE_URL+"/sounds/"+b+".ogg"];
this.playSound(c,a)},handleSoundElements:function(b){var a=this;if($(b).is("[data-play-sound]")){a.playLibrarySound($(b).data("play-sound"),{appendTo:b})
}else{$("[data-play-sound]",b).each(function(){a.playLibrarySound($(this).data("play-sound"),{appendTo:b})})}},_globalHandleAjaxComplete:function(e,f,c){var b=false;
...
},_initRoutes:function(){this.addPageRouteLoader("listpane",this.loadRoute.bind(this));this.addPageRouteLoader("page",this.loadRoute.bind(this));
this.addPageRouteLoader("ticket",(function(c){c.forTypename="ticket";var b=c.url.match(/tickets\/([0-9]+)/);var d=b[1];c.tabLoad=function(){DeskPRO_Window.getMessageBroker().sendMessage("ui.ticket.opened",{ticketId:d})
};c.tabUnload=function(){DeskPRO_Window.getMessageBroker().sendMessage("ui.ticket.closed",{ticketId:d})};this.loadRoute(c)
}).bind(this));this.addPageRouteLoader("person",this.loadRoute.bind(this));this.addPageRouteLoader("kb_article_view",this.loadRoute.bind(this));
...
var a=this},_initWindowInterface:function(){var c=this;this.keyboardShortcuts=new DeskPRO.Agent.KeyboardShortcuts();var d=new DeskPRO.Agent.WindowElement.MainMenuOpener();
$("#user_settings_link").click(function(){var e=new DeskPRO.UI.Overlay({contentMethod:"iframe",iframeUrl:BASE_URL+"agent/settings"});
...
if(c.volume==0||c.volume==0){c.volume=0;$("#sound_icon").addClass("off")}else{$("#sound_icon").removeClass("off")}$("audio").each(function(){this.volume=c.volume
})}});var b=function(){$("#volume_controls_back").hide();$("#volume_controls").fadeOut()};$("#volume_controls_back").click(function(e){e.stopPropagation();
b()});var a=function(){$("#volume_controls_back").show();$("#volume_controls").css({top:30,left:$("#sound_icon").offset().left-7});
$("#volume_controls").fadeIn()};$("#sound_icon").click(function(e){e.preventDefault();e.stopPropagation();a()});this.createMenu=new DeskPRO.UI.Menu({triggerElement:"#create_content_trigger",menuElement:"#create_content_menu"});
...
this.newOrganizationLoader=new DeskPRO.Agent.Widget.BackgroundPopout({loadUrl:BASE_URL+"agent/organizations/new",tabRoute:"page:"+BASE_URL+"agent/organizations/new"});
...
$("#create_ticket_btn").click(function(){DeskPRO_Window.newTicketLoader.toggle()});$("#create_person_btn").click(function(){DeskPRO_Window.newPersonLoader.toggle()
});$("#create_organization_btn").click(function(){DeskPRO_Window.newOrganizationLoader.toggle()});$("#create_article_btn").click(function(){DeskPRO_Window.newArticleLoader.toggle()
...
$("#DP-InterfaceSwitcher > .DP-adminSwitch > .adminSwitcher").click(function(i){i.preventDefault();i.stopPropagation();var k=$(this).offset();
var e=$(this).outerWidth();var g=$(this).outerHeight();var j=$("#interfacesToggle");j.hide().detach().appendTo("body");j.css({top:k.top,left:k.left});
j.show();var f=$('<div class="backdrop" />').appendTo("body");f.click(function(){j.hide();f.remove()})})},toggleAgentStatus:function(a){var b=$("#agent_status");
a=a||false;if(a||b.is(".off")){$("#agent_status_away_overlay").remove();b.removeClass("off");$.ajax({url:BASE_URL+"agent/misc/set-agent-status/available",type:"GET"})
}else{var c=$('<div id="agent_status_away_overlay" />').appendTo("body");b.addClass("off");$.ajax({url:BASE_URL+"agent/misc/set-agent-status/away",type:"GET"})
}},_initSections:function(){var a=this;var b=null;$("#dp_nav [data-section-handler]").each(function(){var e=$(this);if(!e.attr("id")){e.attr("id",Orb.getUniqueId("section_"))
...
c.addClass("dpNavActive");$("#dp_source > section.on").removeClass("on");$("#dp_list > section.on").removeClass("on");$("#dp_list_loading, #dp_source_loading").addClass("on");
if(this.openSection){this.openSection.fireEvent("afterhide")}$("#dp_list_loading").removeClass("on");d.fireEvent("show");
...
this.layout.doResize();this.pageTabStrip=new DeskPRO.Agent.TabStrip($("#tabNavigationPane > .deskproTabList > ul"),new DeskPRO.Agent.TabManager("#dp_content_wrap"));
...
})},_initInterfaceServices:function(){var self=this;this.popover_inited={};$(document).delegate("[data-route]","click",function(ev){if($(this).is(".as-popover")){return
}ev.preventDefault();self.runPageRouteFromElement($(this))});$("#dp_content_wrap").delegate(".as-popover","click",function(ev){ev.preventDefault();
ev.stopPropagation();self._initInterfacePopover($(this)).toggle()});$(document).delegate(".person-tip","mouseover",function(){var el=$(this);
var tipUrl=BASE_URL+"agent/person/"+el.data("person-id")+"/tip";el.addClass("tipped");el.attr("data-tipped",tipUrl);el.attr("data-tipped-options",'ajax:true, showOn: "click", hideOn: { element: "target", event: "click" }, hideOnClickOutside: true ');
el.click(function(ev){Tipped.toggle(this)});if(el.is(".with-route")){el.addClass("cancel-route")}if(el.parent().is(".with-route")){el.parent().addClass("cancel-route")
}});$(document).delegate(".tipped","mouseover",function(){if($(this).is(".tipped-inited")){return}var options={};if($(this).data("tipped-options")){eval("options = {"+$(this).data("tipped-options")+"}")
}Tipped.create(this,$(this).data("tipped")||$(this).attr("title"),options);$(this).addClass("tipped-inited")})},_initInterfacePopover:function(b,c){var i=this;
var a=this.popover_inited;var h=b.data("route");var d=i.parseRoute(h);var e;if(!a[h]){e=new DeskPRO.Agent.PageHelper.Popover({pageUrl:d.url,tabRoute:h,loadTimeout:(b.is(".preload")?1500:0)});
a[h]={count:0,popover:e};e.addEvent("close",function(){if(a[h].count<1){a[h].popover.destroy();delete a[h]}});e.addEvent("destroy",function(){delete a[h]
})}else{e=a[h].popover}console.log(e);a[h].count++;var g=b.closest(".with-page-fragment");if(g.length){var f=g.data("page-fragment");
f.addEvent("destroy",function(){a[h].count--})}else{e.options.destroyOnClose=true}return e},initInterfaceServices:function(b){var a=this;
var c=false;if(b.is(".with-page-fragment")){c=b.data("page-fragment")}else{var e=b.closest(".with-page-fragment");c=e.data("page-fragment")
}$(".as-popover.preload",b).each(function(){var f=a._initInterfacePopover($(this))});if(c){var d=$(".with-scrollbar",b).first();
if(d.length){this.scrollerHandler=new DeskPRO.Agent.ScrollerHandler(c,d,{showEvent:"show",hideEvent:"hide"})}}DeskPRO.ElementHandler_Exec(b);
$(".timeago",b).timeago()}});function toggle_visibility(c,f){var d=$("#"+c);if(d.is(":visible")){d.hide()}else{if(!d.is(".has-init")){d.addClass("has-init");
if(f){var b=$("#"+f);var e=b.offset();d.detach().appendTo("body");d.css({position:"absolute",top:e.top,left:e.left,width:200,height:"auto","z-index":10001})
}}var a=$('<div class="backdrop" />');a.css("z-index",10000);a.appendTo("body");a.click(function(){d.hide();a.remove()});
d.show()}}Orb.createNamespace("DeskPRO.Agent.Layout");DeskPRO.Agent.Layout.DeskproWindow=Orb.Class({Implements:[Orb.Util.Events],initialize:function(){var a=this;
this.LEFT_START=215;this.CENTER_START=55;$(window).resize(function(){if(a._resizeTimeout){window.clearTimeout(a._resizeTimeout)
}a._resizeTimeout=a.doResize.delay(300,a)})},doResize:function(){var b=$(window).width();var a=b-this.LEFT_START-this.CENTER_START;
var c=a*0.4;if(c<370){c=370}else{if(c>700){c=700}}$("#dp_list").width(c);$("#dp_omnibox_wrap").width(c-1);$("#dp_content").css("left",this.LEFT_START+c+1);
this.fireEvent("resized",[this])}});Orb.createNamespace("DeskPRO.Agent.Layout");DeskPRO.Agent.Layout.WindowLayout=Orb.Class({Implements:[Orb.Util.Events],initialize:function(){var c=this;
...
a.html='<div id="'+a.wrapperId+'" class="tabViewDetailContent">'+a.html+"</div>";this.tabs[b]=a;this.fireEvent("addTab",[a,this]);
if(!this.currentTabId||this.options.activateNew){this.activateTab(b)}},activateTab:function(d){if(this.tabs[d]==undefined){console.warn("Unknown tab: %s",d);
return false}if(d==this.currentTabId){return}this.isActivating=true;if(this.currentTabId){this.deactivateCurrentTab()}var b=this.tabs[d];
if(b.isInserted&&b.hideMode=="hide"){console.log("Re-showing tab node: %s",d);var c=$("#"+b.wrapperId,this.containerEl).show();
if(b.callback_reinsert!==undefined){b.callback_reinsert(b,$("#"+b.wrapperId,this.containerEl),this)}this.fireEvent("activateTabReinsert",[b,c,this])
}else{console.log("Rendering tab content: %s",d);var a=$(b.html).appendTo(this.containerEl);b.isInserted=true;a.show();if(b.callback_render!==undefined){b.callback_render(b,$("#"+b.wrapperId,this.containerEl),this)
...
if(a){this.activateTab(a)}}});Orb.createNamespace("DeskPRO.Agent");DeskPRO.Agent.TabStrip=new Orb.Class({Implements:[Orb.Util.Events,Orb.Util.Options],initialize:function(c,d){window.TAB_STRIP=this;
this.tabStrip=c;this.tabManager=d;this.cancelClickActivate=false;this.uniqueCounter=0;var b=this;this.tabStrip.mouseup(this._tabStripClick.bind(this));
...
var a=(b.tabStrip.parent().width()/2);a=a-(a/4);scroll_el=$("#tabNavigationPane .deskproTabList");$("#tabNavSelectorLeft").click(function(f){f.preventDefault();
f.stopPropagation();scroll_el.animate({scrollLeft:"-="+a},200)});$("#tabNavSelectorRight").click(function(f){f.preventDefault();
f.stopPropagation();scroll_el.animate({scrollLeft:"+="+a},200)});this.tabStrip.mousewheel(function(f,g){if(g>0){scroll_el.animate({scrollLeft:"+="+a},100)
}else{scroll_el.animate({scrollLeft:"-="+a},100)}});var e=$('<ul id="dp_tabstrip_menu" />').hide().appendTo("body");e.delegate("li","mouseover",function(){$("li.over",c).removeClass("over");
$("#"+$(this).data("tab-el-id")).addClass("over")});e.mouseout(function(){$("li.over",c).removeClass("over")});this.tabMenu=new DeskPRO.UI.Menu({triggerElement:$("#tabDropdownPicker"),menuElement:e,onBeforeMenuOpened:function(){e.empty();
$("li",c).each(function(){var g=$("a",this).clone();var f=$("<li />");f.data("tab-id",$(this).data("tab-id"));f.data("tab-el-id",$(this).attr("id"));
f.append(g);if($(this).is(".activeTabList")){f.addClass("highlight")}e.append(f)})},onItemClicked:function(j){var i=$(j.itemEl);
var g=i.data("tab-id");var k=$("#"+g);b.activateTabById(g);var k=$("#tabNavigationPane li.activeTabList");if($("#tabNavigationPane").is(".with-overflow")&&k.length){var h=k.position().left;
var m=k.width();var f=scroll_el.width();var l=scroll_el.scrollLeft();if(h<l||(h+m)>(l+f)){scroll_el.scrollLeft(h)}}},onClose:function(){$("li.over",c).removeClass("over")
}})},alertTab:function(a){var b=$("li."+a,this.tabStrip);if(!b.length||b.is(".activeTabList")||b.is(".is-alerting")){return
...
if(c.title){d.meta.title=c.title}if(c.forTypename){d.LOADING_TYPENAME=c.forTypename}var e=this.addTab(d);if(c.tabLoad){c.tabLoad()
}return e},_tabStripClick:function(e){if(this.cancelClickActivate){this.cancelClickActivate=false;return}this.cancelClickActivate=true;
var b=$(e.target);if(b.is("li")){var d=b}else{if(b.parent().is("li")){var d=b.parent()}else{var d=b.parentsUntil("li");if(d.length){d=d.parent()
}}}if(!d.is("li")){console.log("not click %o",e.target);return}e.preventDefault();e.stopPropagation();var a=d.data("tab-id");
if(b.is(".close")||e.which==2||e.isDbl){var c=this.getTabById(a);if(c.page&&c.page.fireEvent){e.deskpro={cancelClose:false};
c.page.fireEvent("closeTab",[e,c]);if(e.deskpro.cancelClose){return}}c.isCloseClick=true;this.tabManager.removeTab(a);c.isCloseClick=false;
...
if(b.page.TYPENAME!="basic"){d+=" "+b.page.TYPENAME}if(b.page.LOADING_TYPENAME){d+=" "+b.page.LOADING_TYPENAME}if(b.page.getMetaData("tabTip")){if(b.page.getMetaData("tabTip").indexOf(".")===0){b.page.setMetaData("fetchTabTip",true);
...
}this.recalculateScrolling()},recalculateScrolling:function(){var c=$("#tabNavigationPane");var a=0;var b=$("li",this.tabStrip).each(function(){a+=$(this).outerWidth()
}).length;a+=21*b;this.tabStrip.width(a);var e=c.is(".with-overflow");var d=(c.width()<a);if(!e&&!d){}else{if(e&&!d){console.debug("[TabStrip] Remove scrolling");
c.removeClass("with-overflow");this.tabStrip.scrollLeft(0)}else{if(!e&&d){console.debug("[TabStrip] Add scrolling");c.addClass("with-overflow")
}}}},_onTabDeactivate:function(a,b,c){$("#tiptip_holder").clearQueue().hide()},_onTabActivate:function(b){$("li",this.tabStrip).removeClass("activeTabList");
...
$(document).bind("keydown","o",this.showNewOrganization.bind(this));this.boundShortkuts={};this.addContextShortcut("ticket","ctrl+shift+r","shortcutFocusReply")
...
},showNewPerson:function(){DeskPRO_Window.newPersonLoader.toggle()},showNewOrganization:function(){DeskPRO_Window.newOrganizationLoader.toggle()
...
}}});Orb.createNamespace("DeskPRO.Agent.PageFragment");DeskPRO.Agent.PageFragment.Basic=new Orb.Class({Implements:[Orb.Util.Events],initializeProperties:function(){},initialize:function(b){var a=this;
this.pageUid=Orb.uuid();this.ZONE="agent";this.TYPENAME="basic";this.allowDupe=false;this.scripts=[];this.stylesheets=[];
this.html="";this.meta={};this.urls={};this.destroyObjects=[];this.featureSelectors={routes:[],times:[]};this.initializeProperties();
if(b){this.html=b}this.addEvent("activate",function(){DeskPRO_Window.getMessageBroker().sendMessage("page-fragment.activated",{page:this})
},this);this.addEvent("deactivate",function(){DeskPRO_Window.getMessageBroker().sendMessage("page-fragment.deactivated",{page:this})
},this);this.addEvent("render",function(c){a.wrapper=c;c.data("page-fragment",a);c.addClass("with-page-fragment");DeskPRO_Window.initInterfaceServices(c);
if(!this.noDeleteHtmlString){delete this.html}},this);var a=this;this.addEvent("activate",this.activate);this.addEvent("deactivate",this.deactivate);
this.addEvent("render",this.initPage);this.addEvent("destroy",this.destroyPage);this.init();this.addEvent("destroy",function(){a.wrapper.data("with-page-fragment",null);
var c;for(c=0;c<this.destroyObjects.length;c++){this.destroyObjects[c].destroy()}this.destroyObjects=null},this);this.addEvent("destroy",this.destroy)
},init:function(){},activate:function(){},deactivate:function(){},ownObject:function(a){if(a.destroy){this.destroyObjects.push(a)
...
},initPage:function(a){this.wrapper=a},destroyPage:function(){},getEl:function(b){if(this.getMetaData("baseId")){b=this.getMetaData("baseId")+"_"+b
}var a=null;if(this.wrapper){a=this.wrapper}return $("#"+b,a)},closeSelf:function(){DeskPRO_Window.removePage(this)},destroy:function(){}});
Orb.createNamespace("DeskPRO.Agent.PageFragment.Page");DeskPRO.Agent.PageFragment.Page.Loading=new Orb.Class({Extends:DeskPRO.Agent.PageFragment.Basic,initializeProperties:function(){this.allowDupe=true;
this.TYPENAME="loading"},initPage:function(a){this.wrapper=$(a)}});Orb.createNamespace("DeskPRO.Agent.Notifier");DeskPRO.Agent.Notifier.Notifier=new Class({Implements:[Events,Options],options:{notifySummaryButton:null,notifyList:null},button:null,list:null,notifyTypes:[],initialize:function(b){if(b){this.setOptions(b)
...
var a=this;$("span.dismiss",this.list).live("click",function(){a.handleDismissClick($(this))});Array.each(this.notifyTypes,function(c){c.addEvent("listUpdated",this.updateListForType,this)
...
DeskPRO.Agent.WindowElement.TabWatcher.Tickets=new Orb.Class({Implements:[Orb.Util.Events],initialize:function(){this.addEvent("activateTab",this.activateTab,this);
this.addEvent("deactivateTab",this.deactivateTab,this);this.releasing={};DeskPRO_Window.getMessageBroker().addMessageListener("agent-notification.tickets.unlocked",function(a){var b=a.ticket_id;
...
}})},this)},activateTab:function(a){var b=a.page.getMetaData("ticket_id");if(this.releasing[b]){this.releasing[b].abort();
