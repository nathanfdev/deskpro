...
$("#page_loading").remove();$("#loading_css").remove();this.hashRouter=window.DeskPRO_HashRouter;this.hashRouter.setBaseUrl(BASE_URL);
var a=this;console.log("ere");$.history.init(function(b){a.loadHashPath(b)},{unescape:",/:"})},loadHashPath:function(f){var d=this.pageTabStrip.findTabByAnchor(f);
if(d){this.pageTabStrip.activateTabById(d);return}var e=f.split(":");if(e.length!=2&&e.length!=3){return}if(e.length==2){var a=e[0];
var d=null;var c=e[1]}else{var a=e[0];var d=e[1];var c=e[2]}c=c.split(":");if(!this.hashRouter.hasAnchor(a)){return}var b=this.hashRouter.getUrl(a,c);
this.loadPage(b,{anchor:f})},windowStateUpdated:function(a){return;if(this.DEBUG.disableSaveState){return}this.winStateQueue.include(a);
if(this.saveWindowState_timeout){window.clearTimeout(this.saveWindowState_timeout)}this.saveWindowState_timeout=this.saveWindowState.delay(4500,this)
...
};var b=null;if(a(".Kb")){b=this.sections.kb_section}else{if(a(".Ticket")){b=this.sections.tickets_section}else{if(a(".People")||a(".Org")){b=this.sections.people_section
}else{if(a(".AgentChat")){b=this.sections.agent_chat_section}else{if(a(".OpenChats")){b=this.sections.chat_section}else{if(a(".Idea")){b=this.sections.ideas_section
}}}}}}if(!b){console.error("List page fragment has no section: %s: %o",c.getMetaData("fragmentClass",""),c);return}b.setListPageFragment(c)
},getListPage:function(){return this.listPage},addPageTab:function(a){this.pageTabStrip.addTab(a)},removePage:function(b){var a=this.pageTabStrip.findTabByPage(b);
if(a){this.pageTabStrip.removeTabById(a)}a=false;a=this.listTabStrip.findTabByPage(b);if(a){this.listTabStrip.removeTabById(a)
...
default:this.loadPage(a.url,a);break}},loadListPane:function(a,b,c){this._doAjaxLoadRoute(a,b,(function(e){this.stopLoadingIndicator();
...
return}if(c&&!c.ignoreExist){var a=this.pageTabStrip.getTabByRouteUrl(b);if(a&&!(a.page&&a.page.allowDupe)){this.pageTabStrip.activateTabById(a.id);
...
}}if(c.anchor){e.setMetaData("anchor",c.anchor)}this.addPageTab(e);if(d){d(e)}}).bind(this))},_doAjaxLoadRoute:function(b,c,a){if(!b){console.error("No URL provided! routeData: %o",c);
return}if(!c||!c.tabPlaceholderId){this.startLoadingIndicator()}if(c&&c.postData){$.ajax({dataType:"text",url:b,type:"POST",data:c.postData,success:(function(d){this.stopLoadingIndicator();
a(d)}).bind(this)})}else{$.ajax({dataType:"text",url:b,type:"GET",success:(function(d){this.stopLoadingIndicator();a(d)}).bind(this)})
}},createPageFragment:function(html,classname){pageMeta={title:false,fragmentClass:classname||"DeskPRO.Agent.PageFragment.Basic"};
...
return page},reloadSelectedTab:function(){var b=this.pageTabStrip.getActiveTab();if(!b){return}var a=b.page.meta.routeData.route;
...
});b.push("</audio>");b=b.join("");var c=$(b);if(options.destroyAfter){c.bind("ended",function(){$(this).remove()})}c.appendTo("body");
return c},playLibrarySound:function(b,a){var c=[ASSETS_BASE_URL+"sounds/"+b+".mp3",ASSETS_BASE_URL+"sounds/"+b+".ogg"];this.playSound(c,a)
},handleSoundElements:function(b){var a=this;$("[data-play-sound]",b).each(function(){a.playLibrarySound($(this).data("play-sound"))
})},startLoadingIndicator:function(a){if(!a){a=1}this.loadingIndicatorCount+=a;this._showHideLoadingIndicator()},stopLoadingIndicator:function(a){if(!a){a=1
}this.loadingIndicatorCount-=a;if(this.loadingIndicatorCount<0){this.loadingIndicatorCount=0}this._showHideLoadingIndicator()
},_showHideLoadingIndicator:function(){if(this.loadingIndicatorCount>0){if(!this.loadingIndicatorEl){this.loadingIndicatorEl=$('<div class="window-loading-indicator" style="display:none" />');
this.loadingIndicatorEl.appendTo("body")}this.loadingIndicatorEl.css({position:"absolute",top:0,left:($(document).width()/2)-this.loadingIndicatorEl.outerWidth(),"z-index":100000});
this.loadingIndicatorEl.slideDown(250)}else{if(this.loadingIndicatorEl){this.loadingIndicatorEl.stop().slideUp(150)}}},_globalHandleAjaxComplete:function(e,f,c){var b=false;
...
var d=parseInt(a.html())||0;d++;a.html(d)}},_globalHandleAjaxError:function(c,i,a,g){this.stopLoadingIndicator(1000);if(i.status=="0"||(i.statusText&&i.statusText=="abort")){return
}var d=i.responseText;try{d=$.parseJSON(d)}catch(f){d=null}if(i&&i.status&&i.status=="403"){if(d&&d.error&&d.error=="session_expired"){window.location=d.redirect_login;
a.error=null;a.complete=null;this.showStatusMessage("Your session has timed out, you must log in");return}if(d&&d.error&&d.error=="not_allowed"){this.showStatusMessage("The action you attempted to execute is not allowed:<br />"+d.errorMessage);
return}}if(a&&a.error){return}var h=null;if(d&&d.sn){h=d.sn}else{var b=/\[\[SN:(.*?)\]\]/.exec(i.responseText);if(b){h=b[1]
}}console.log(h);if(h){this._showAjaxError("If the error persists, give your administrator this code: SN"+h)}else{this._showAjaxError()
...
this.messageChanneler.subscribeChannel("tickets.new-tickets");this.messageChanneler.subscribeChannel("tickets.new-messages");
this.messageChanneler.subscribeChannel("tickets.updated");this.poller=new DeskPRO.AjaxPoller.MessagePoller(this.messageBroker,{ajaxUrl:BASE_URL+"agent/poller",interval:5000});
...
this.addPageRouteLoader("listpane",this.loadRoute.bind(this));this.addPageRouteLoader("page",this.loadRoute.bind(this));this.addPageRouteLoader("ticket",this.loadRoute.bind(this));
this.addPageRouteLoader("person",this.loadRoute.bind(this));this.addPageRouteLoader("kb_article_view",this.loadRoute.bind(this));
...
$("#volume_controls").position({my:"center top",at:"center bottom",of:$("#sound_icon")});$("#volume_controls").fadeIn()};
$("#sound_icon a").click(function(e){e.preventDefault();a()})},toggleAgentStatus:function(a){var b=$("#agent_status");a=a||false;
if(a||b.is(".off")){$("#agent_status_away_overlay").remove();b.removeClass("off");$.ajax({url:BASE_URL+"agent/misc/set-agent-status/available",type:"GET"})
}else{var c=$('<div id="agent_status_away_overlay" />').appendTo("body");c.click(function(d){d.preventDefault();d.stopPropagation()
});b.addClass("off");$.ajax({url:BASE_URL+"agent/misc/set-agent-status/away",type:"GET"})}},_initSections:function(){var a=this;
var b=null;$("#deskpro_sections [data-section-handler]").each(function(){var e=$(this);if(!e.attr("id")){e.attr("id",Orb.getUniqueId("section_"))
...
$("#deskpro_outline > section.on").removeClass("on");$("#deskpro_list > section.on").removeClass("on");if(this.openSection){this.openSection.fireEvent("afterhide")
}d.fireEvent("show");var a=d.getSectionElement();if(a){a.addClass("on")}var e=d.getListElement();if(e){e.addClass("on")}d.fireEvent("aftershow");
...
this.LEFT_START=255;$(window).resize(function(){if(a._resizeTimeout){window.clearTimeout(a._resizeTimeout)}a._resizeTimeout=a.doResize.delay(400,a)
})},doResize:function(){var b=$(window).width();var a=b-this.LEFT_START;var c=a*0.35;if(c<255){c=255}$("#deskpro_list").width(c);
...
}c._resizeTimeout=c.handleResize.delay(400,c)})},calculateHeight:function(a){a-=this.TOP_POS;a-=(4*this.PADDING);return a
...
return false}});return a},findTabByAnchor:function(b){var a=false;Object.each(this.tabManager.getTabs(),function(c){if(c.page.getMetaData("anchor")==b){a=c.id;
return false}});return a},getTabByAnchor:function(b){var a=this.findTabByAnchor(b);if(!a){return null}return this.getTabById(a)
...
},getTabById:function(a){return this.tabManager.getTab(a)},addTab:function(a){console.log(a);var b="tab"+this.uniqueCounter++;
this.tabManager.addTab(b,{html:a.getHtml(),page:a,title:a.getMetaData("title","Untitled"),callback_render:function(e,c,d){c=$(c);
...
d.meta.loadingUrl=a;d.meta.loadingRouteData=c;var e=this.addTab(d);return e},resizeTabListWidth:function(){var a=0;$("> li",this.tabStrip).each(function(){a+=$(this).outerWidth()
...
}}this.tabManager.removeTab(a);return}this.tabManager.activateTab(a);this.cancelClickActivate=false},_onTabAdd:function(b){b.btnId=b.id;
if(!b.page.getMetaData("anchor")&&b.page.TYPENAME!="loading"){b.page.setMetaData("anchor",b.id)}var c=b.page.getMetaData("tabIdClass","");
...
d+='" data-tipped="'+b.btnId+"_tip\" data-tipped-options=\"inline: true, skin: 'light', showDelay: 1, hook: 'topmiddle'\">"
}else{d+='" data-tipped="'+b.tabTip+"\" data-tipped-options=\"skin: 'light', showDelay: 1, hook: 'topmiddle'\">"}}else{d+='" data-tipped="'+b.title+"\" data-tipped-options=\"skin: 'light', showDelay: 1, hook: 'topmiddle'\">"
}d+='<a class="link">'+b.title+"</a>";d+='<a class="close-tab">Close</a>';d+="</li>";var a=$(d);if(b.page&&b.page.meta.tabPlaceholderId){var e=this.getTabById(b.page.meta.tabPlaceholderId);
var f=e.btnId;this.removeTabById(b.page.meta.tabPlaceholderId);$("#"+f).replaceWith(a)}else{a.appendTo(this.tabStrip);this.resizeTabListWidth()
}},_onTabDeactivate:function(a,b,c){$("#tiptip_holder").clearQueue().hide()},_onTabActivate:function(b){$("li",this.tabStrip).removeClass("active-tab");
var d=$("#"+b.btnId,this.tabStrip).addClass("active-tab");if(d.is(".is-alerting")){d.removeClass("alert-highlight").removeClass("is-alerting");
...
var a=b.btnId+"_tip";if(!document.getElementById(a)){$("#"+b.wrapperId+" "+b.page.getMetaData("tabTip")).attr("id",a)}}if(b.page.getMetaData("anchor")){jQuery.history.load(b.page.getMetaData("anchor"))
}},_onTabRemove:function(a){$("#tiptip_holder").clearQueue().hide();if(a.page&&a.page.TYPENAME&&a.page.TYPENAME=="loading"){return
}$("#"+a.btnId).remove();this.resizeTabListWidth()}});Orb.createNamespace("DeskPRO.Agent");DeskPRO.Agent.ScrollerHandler=new Orb.Class({Implements:[Orb.Util.Options],initialize:function(d,c,b){this.pageObject=d;
...
a.resize(function(){b.tinyscrollbar_update()})},removeResizeTimer:function(){this.element.unbind("resize")}});Orb.createNamespace("DeskPRO.Agent.PageFragment");
DeskPRO.Agent.PageFragment.Basic=new Class({Implements:[Events,DeskPRO.Agent.Widgetable],pageUuid:null,ZONE:"agent",TYPENAME:"basic",allowDupe:false,scripts:[],stylesheets:[],html:"",meta:{},urls:{},featureSelectors:{routes:[],times:[]},initialize:function(c){this.pageUid=Orb.uuid();
