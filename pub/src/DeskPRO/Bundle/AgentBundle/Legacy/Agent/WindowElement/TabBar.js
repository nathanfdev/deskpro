...
DeskPRO.Agent.WindowElement.TabBar = new Orb.Class({
  ...
  addTab: function(page) {
    ...
    if (page.meta.existingWrapper) {
      ...
      data.wrapper.appendTo(this.getBodyPane());
    } else {
      var preparedOutput = DeskPRO_Window.prepareWidgetedHtml(page.getHtml());

      data.wrapper = $('<div id="'+data.wrapperId+'" class="tabViewDetailContent" style="display: none">' + preparedOutput.html + '</div>').appendTo(this.getBodyPane());
