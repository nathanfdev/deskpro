import React from 'react';
import PortalPhrases from 'DeskPRO/Bundle/PortalBundle/PortalPhrases';

export default class PortalAttach extends React.Component {

  onClickAddAttachment = event => {
    event.preventDefault();
  };

  onClick = event => {
    event.preventDefault();
  };

  render() {
    return (
       <div className="new-ticket-attachements">
          <a href="#" className="attach-file" onClick={this.onClickAddAttachment}>
            <i className="fa fa-upload" />
            <span className="text">Drag a file in here or</span>
            <span className="fake-button">Choose a file</span>
          </a>

          <div className="title">Attached files:</div>

          <ul>
            <li>
                {/** these icons will be in the payload from the server (HTML) **/}
              <a href="#" onClick={this.onClick}>
                <i className="fa fa-file-text-o" /> Screen Shot 2014-09-30 at 6.14.15 PM.png
              </a>
              <span className="file-size">(245kb)</span>
              <a href="#" className="remove-attachement" onClick={this.onClick}>
                <i className="fa fa-times" />{PortalPhrases.get('portal.general.delete')}
              </a>
            </li>

            <li>
              <a href="#" onClick={this.onClick}>
                <i className="fa fa-file-text-o" /> a_doc_file.doc
              </a>
              <span className="file-size">(84kb)</span>
              <a href="#" className="remove-attachement" onClick={this.onClick}>
                <i className="fa fa-times" />{PortalPhrases.get('portal.general.delete')}
              </a>
            </li>
          </ul>
      </div>
    );
  }
}
