...
namespace DeskPRO\Bundle\AppBundle\TermEngine\Term\TicketDateLastAgentReply;
...
class PhpTicketDateLastAgentReplyTermCompiler extends AbstractPhpTermCompiler
{
    /**
    ...
    protected function doCompile(TermInterface $term)
    {
        ...
        if (TermInterface::OP_RANGE == $op) {
            return new PhpCheck(
                'ticket.date_last_agent_reply >= :date1 and ticket.date_last_agent_reply <= :date2',
                array(
                    'op' => $op,
                    'date1' => $date1,
                    'date2' => $date2
            ...
            return new PhpCheck(
                'ticket.date_last_agent_reply < :date1 or ticket.date_last_agent_reply > :date2',
                array(
                    'op' => $op,
                    'date1' => $date1,
                    'date2' => $date2
            ...
            return new PhpCheck(
                'ticket.date_last_agent_reply '. $real_op .' :date',
                array(
                    'date' => $date1
                )
            );
