...
class ServerReqsController extends AbstractController
{
	...
	public function getAction()
	{
		...
		$web_checks = $this->_generateMessages($this->server_check->getErrors());
		...
		if (file_exists(dp_get_data_dir() . '/cli-server-reqs-check.dat')) {
			...
			$cli_checks = $this->_generateMessages($data['checks']);
		}
	...
	protected function _generateCheckTable()
	{
		...
		// @TODO should be extracted to some service, looks not nice
		...
		$this->checksTable = array(
			...
			'apc_check' => array(
				'description'    => ($is_win)? 'Checking for the <a href="http://www.php.net/manual/en/apc.installation.php">APC extension</a> or the <a href="http://www.php.net/manual/en/book.wincache.php">WinCache extension</a>' : 'Checking for the <a href="http://www.php.net/manual/en/apc.installation.php">APC extension</a>',
