...
class TicketsStep extends AbstractDeskpro3Step
{
	...
	protected function processTicketInfo($all_ticket_info)
	{
		...
		foreach ($ticket_logs as $tlog) {
			...
			if ($tlog['techid']) {
				$insert_tlog['person_id'] = $this->getMappedNewId('tech', $tlog['techid']);
				if (!$insert_tlog['person_id']) {
					continue;
				}
			} elseif ($tlog['userid']) {
				$insert_tlog['person_id'] = $this->getMappedNewId('user', $tlog['userid']);
				if (!$insert_tlog['person_id']) {
					continue;
				}
			} else {
			...
			switch ($tlog['actionlog']) {
				...
				case 'category':
					$old_id = $new_id = 0;
					if ($tlog['id_before']) {
						$old_id = $this->getMappedNewId('ticket_category', $tlog['id_before']);
						if (!$old_id) {
							break;
						}
					}
					...
					if ($old_id) {
						...
						$insert_tlog['details']['old_department_title'] = $this->getThingTitle('department', $old_id);
					} else {
					...
					if ($new_id) {
						...
						$insert_tlog['details']['new_department_title'] = $this->getThingTitle('department', $new_id);
					} else {
				...
				case 'workflow':
					...
					if ($old_id) {
						...
						$insert_tlog['details']['old_workflow_title'] = $this->getThingTitle('ticket_workflow', $old_id);
					} else {
					...
					if ($new_id) {
						...
						$insert_tlog['details']['new_workflow_title'] = $this->getThingTitle('ticket_workflow', $new_id);
					} else {
				...
				case 'priority':
					...
					if ($old_id) {
						...
						$insert_tlog['details']['old_priority_title'] = $this->getThingTitle('ticket_priority', $old_id);
						$insert_tlog['details']['old_priority_pri']   = $this->getThingTitle('ticket_priority_pri', $old_id);
					} else {
					...
					if ($new_id) {
						...
						$insert_tlog['details']['new_priority_title'] = $this->getThingTitle('ticket_priority', $new_id);
						$insert_tlog['details']['new_priority_pri']   = $this->getThingTitle('ticket_priority_pri', $new_id);
					} else {
				...
				case 'tech':
					...
					if ($tlog['id_before']) {
						$old_id = $this->getMappedNewId('tech', $tlog['id_before']);
						if (!$old_id) {
							break;
						}
				...
				case 'tech_participant_add':
					$new_id = $this->getMappedNewId('tech', $tlog['id_before']);
					if (!$new_id) {
						break;
					}
					$new_agent_info = $this->getPersonInfo($new_id);
				...
				case 'user_participant_add':
					...
					$new_id = $this->getMappedNewId('user', $tlog['id_before']);
					if (!$new_id) {
						break;
					}
				...
				case 'company':
					...
					if ($tlog['id_before']) {
						$id_before = $this->getMappedNewId('company', $tlog['id_before']);
						if (!$id_before) {
							break;
						}
						$info_before = $this->getEm()->find('DeskPRO:Organization', $id_before);
					}
					if ($tlog['id_after']) {
						$id_after = $this->getMappedNewId('company', $tlog['id_after']);
						if (!$id_after) {
							break;
						}
						$info_after = $this->getEm()->find('DeskPRO:Organization', $id_after);
					}
				...
				case 'changed_person':
					...
					if ($tlog['id_before']) {
						$old_id = $this->getMappedNewId('user', $tlog['id_before']);
						if (!$old_id) {
							break;
						}
				...
				case 'email_user':
					...
					$id = $this->getMappedNewId('user', $tlog['id_before']);
					if (!$id) {
						break;
					}
					$p = $this->getPersonInfo($id);
				...
				case 'custom':
					$field_id = $this->getMappedNewId('ticket_def_name', $tlog['extra']);
					if (!$field_id) {
						break;
					}
	...
	protected function getThingTitle($thing, $id)
	{
		switch ($thing) {
			case 'department':
