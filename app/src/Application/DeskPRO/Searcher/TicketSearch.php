		...
		foreach ($this->terms as $term => $info) {
			...
			switch ($term) {
				case self::TERM_ID:
					...
					$this->summary[] = $this->_rangeSummary($tr->phrase('global.id'), $op, $choice);
					break;
				...
				case self::TERM_CATEGORY:
					...
					$this->summary[] = $this->_choiceSummary($tr->phrase('global.category'), $op, $choice, function($choice) {
				...
				case self::TERM_PRODUCT:
					...
					$this->summary[] = $this->_choiceSummary($tr->phrase('global.product'), $op, $choice, function($choice) {
				...
				case self::TERM_URGENCY:
					...
					$this->summary[] = $this->_rangeSummary($tr->phrase('global.urgency'), $op, $choice);
				...
				case self::TERM_WORKFLOW:
					...
					$this->summary[] = $this->_choiceSummary($tr->phrase('global.workflow'), $op, $choice, function($choice) {
				...
				case self::TERM_AGENT:
					...
					if ($unassigned) {
						$this->summary[] = $this->_choiceSummary($tr->phrase('global.agent'), $op, $tr->phrase('global.unassigned'));
						$wheres[] = "$tickets_table.agent_id IS NULL";
					} else {
						if ($agent_ids) {
							$this->summary[] = $this->_choiceSummary($tr->phrase('global.agent'), $op, $agent_ids, function($choice) {
				...
				case self::TERM_AGENT_TEAM:
					...
					if ($no_team) {
						...
						$this->summary[] = $this->_choiceSummary($tr->phrase('global.agent_team'), $op, $tr->phrase('global.unassigned'));

					} else {
						if ($team_ids) {
							$this->summary[] = $this->_choiceSummary($tr->phrase('global.agent_team'), $op, $team_ids, function($choice) {
						...
						if ($not_ids) {
							$this->summary[] = $this->_choiceSummary($tr->phrase('global.agent_team'), 'not', $not_ids, function($choice) {
								$titles = App::getEntityRepository('DeskPRO:AgentTeam')->getTeamNames((array)$choice);
								return $titles;
							});
				...
				case self::TERM_HIDDEN_STATUS:
					...
					$this->summary[] = $tr->phrase('agent.x_is_y', array('field' => $tr->phrase('global.status'), 'value' => $choice_str));
				...
				case self::TERM_ORGANIZATION:
					$this->summary[] = $this->_choiceSummary($tr->phrase('global.organization'), $op, $choice, function($choice) {
				...
				case self::TERM_PARTICIPANT:
					...
					$this->summary[] = $this->_choiceSummary($tr->phrase('global.followers'), $op, $choice, function($choice) {
						$titles = App::getEntityRepository('DeskPRO:Person')->getAgentNames((array)$choice);
						return $titles;
					}, true);
				...
				case self::TERM_SUBJECT:
					...
					if ($op == self::OP_IS) {
						$this->summary[] = $tr->phrase('agent.x_is_y', array('field' => $tr->phrase('global.subject'), 'value' => $choice));
					} else {
						$this->summary[] = $tr->phrase('agent.x_is_not_y', array('field' => $tr->phrase('global.subject'), 'value' => $choice));
					}
				...
				case self::TERM_LABEL:
					...
					$this->summary[] = $this->_choiceSummary($tr->phrase('global.label'), $op, $choice);
