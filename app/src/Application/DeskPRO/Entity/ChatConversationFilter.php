...
/**
 * ChatConversation filters
 */
class ChatConversationFilter extends \Application\DeskPRO\Domain\DomainObject
{
	/**
	 * @var int
	 *
	 ...
	 *
	 * @return Application\DeskPRO\Searcher\ChatConversationSearch
	 */
	public function getSearcher(array $force_terms = array())
	{
		$searcher = new \Application\DeskPRO\Searcher\ChatConversationSearch();
	...
	public function getTitle()
	{
		if ($this->sys_name) {
			...
			switch ($this->sys_name) {
				case 'agent':
				case 'agent_w_hold':
					return $tr->phrase('agent.chat.filter_agent');

				case 'agent_team':
				case 'agent_team_w_hold':

					$phrase = 'agent.chat.filter_agent_team';
					...
					if ($person && $person->isAgent()) {
						...
						if (count($person->getTeams()) > 1) {
							$phrase = 'agent.chat.filter_agent_teams';
						}
					}
				...
				case 'participant_w_hold':
					return $tr->phrase('agent.chat.filter_participant');

				case 'unassigned':
				case 'unassigned_w_hold':
					return $tr->phrase('agent.chat.filter_unassigned');

				case 'all':
				case 'all_w_hold':
					return $tr->phrase('agent.chat.filter_all');
			}
		}
	...
	public function getResults(Person $person = null)
	{
		...
		$order_by = $person->getPref('agent.ui.chat-filter-order-by.' . $this->id);
	...
	public static function loadMetadata(ClassMetadata $metadata)
	{
		...
		$metadata->customRepositoryClassName = 'Application\DeskPRO\EntityRepository\ChatConversationFilter';
		$metadata->setPrimaryTable(array( 'name' => 'chat_conversations_filters', 'uniqueConstraints' => array( 'sys_name_unique' => array( 'columns' => array( 0 => 'sys_name', ), ), ), ));
