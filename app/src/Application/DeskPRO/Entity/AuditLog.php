...
class AuditLog extends DomainObject
{
    ...
    public static function getObjectTypeFromVar($object)
    {
        ...
        } elseif (is_array($object) && isset($object['type']) && isset($object['id'])) {
            return $object['type'];
        } else {
            if (is_object($object)) {
                return get_class($object);
            } elseif (is_string($object) || ctype_digit($object)) {
    ...
    public static function getObjectIdFromVar($object)
    {
        if (is_object($object) && $object instanceof DomainObject) {
            ...
            return $object['id'];
        } elseif (is_string($object) && $id = Strings::extractRegexMatch('#@(\d+)$#', $object)) {
            return $id;
        }
