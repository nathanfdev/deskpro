...
class LdapModel
{
	...
	public function __construct(Usersource $usersource = null)
	{
		if ($usersource) {
			...
			$fields = array('port', 'host', 'baseDn', 'username', 'password', 'field_username', 'field_email', 'accountFilterFormat');
			foreach ($fields as $f) {
				$this->$f = $usersource->getOption($f, null);
			}

			if (!$this->field_username) $this->field_username = 'uid';
			if (!$this->field_email)    $this->field_email = 'mail';
