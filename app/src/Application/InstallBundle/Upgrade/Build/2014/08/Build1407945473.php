...
class Build1407945473 extends AbstractBuild
{
	public function run()
	{
		...
		$this->execMutateSql(
			...
			VALUES ('job_queue', 'job_queue', 'Job Queue', 'Processes the job queue', 'Application\\\\DeskPRO\\\\WorkerProcess\\\\Job\\\\JobQueueExecutor', X'613A303A7B7D', '60', NULL, NULL)
		"
		);
		...
		$this->execMutateSql(
			"
			INSERT IGNORE INTO `worker_jobs` (`id`, `worker_group`, `title`, `description`, `job_class`, `data`, `run_interval`, `last_run_date`, `last_start_date`)
			VALUES ('job_supervisor', 'job_queue', 'Job Supervisor', 'Supervises the job queue', 'Application\\\\DeskPRO\\\\WorkerProcess\\\\Job\\\\JobQueueSupervisor', X'613A303A7B7D', '60', NULL, NULL)
		"
		);
	}
}
