 ...
 */
final class TicketFields extends AbstractFixture implements FixtureDeleteInterface
{
    /**
     * {@inheritdoc}
     */
    ...
    public function delete()
    {
        ...
        foreach ($fields->ticket_fields as $field) {
            try {
                ...
                $this->client->ticketFields()->delete(array('id' => $field->id));
    ...
    protected function createItem($num, DateTime $initial_time, DateTime $end_time)
    {
        ...
        $response = $this->client->ticketFields()->create($params);

        $this->logger->info('Ticket field created successfully');
        $this->logger->debug(json_encode($response->ticket_field));
    }
}
