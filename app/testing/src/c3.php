...
if (!defined('C3_CODECOVERAGE_MEDIATE_INIT')) {
    ...
    function __c3_factory($filename)
    {
        ...
        if (isset($_SERVER['HTTP_X_CODECEPTION_CODECOVERAGE_SUITE'])) {
            ...
            try {
                $settings = \Codeception\Configuration::suiteSettings($suite, \Codeception\Configuration::config(C3_CODECEPTION_CONFIG_PATH));
            } catch (Exception $e) {
                __c3_error($e->getMessage());
            }
        } else {
            $settings = \Codeception\Configuration::config(C3_CODECEPTION_CONFIG_PATH);
        }
...
if (!defined('C3_CODECEPTION_CONFIG_PATH')) {
    __c3_error("Not defined: C3_CODECEPTION_CONFIG_PATH");
}

try {
    \Codeception\Configuration::config(C3_CODECEPTION_CONFIG_PATH);
} catch (\Exception $e) {
    __c3_error($e->getMessage());
}
