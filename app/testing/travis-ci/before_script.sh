...
java -mx512m -jar /tmp/selenium-server-standalone-2.38.0.jar -role hub > /var/log/selenium-hub.log 2>&1 &
echo "."
sleep 3
echo "--> Done"

echo "Starting Selenium Node"
java -mx512m -jar /tmp/selenium-server-standalone-2.38.0.jar -role node -hub http://localhost:4444/grid/register > /var/log/selenium-node.log 2>&1 &
echo "."
sleep 3
echo "--> Done"