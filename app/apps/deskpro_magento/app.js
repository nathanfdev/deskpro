	...
	return {
		init: function() {
			if (this.getSetting('widget_ticket')) {
				this.registerWidgetTab('ticket', '@properties.tab', "Magento ({{content.matches.length}})", 'tab.html', function($scope, $app, $ticket) {
					$scope.matches = [];
			...
			if (this.getSetting('widget_profile')) {
				this.registerWidgetTab('user', '@properties.tab', "Magento ({{content.matches.length}})", 'tab.html', function($scope, $app, $ticket) {
					$scope.matches = [];
