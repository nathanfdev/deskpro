define(['DeskPRO/Util/Strings'], function(Strings) {
	return ['$scope', 'Api', '$q', '$modal', function($scope, Api, $q, $modal) {
		...
		Api.sendGet('/apps/packages/deskpro_slack/check-requirements').then(function(res) {
		...
		['webhook_url'].forEach(function(field) {
		...
		function runTest() {
			...
			Api.sendPostJson('/apps/packages/deskpro_slack/test-settings', postData).then(function(res) {
				deferred.resolve({
					log: res.data.log || '',
		...
		$scope.openTestModal = function(existing_results) {
			...
			var inst = $modal.open({
				templateUrl: 'deskpro_slack/Install/test-settings-modal.html',
