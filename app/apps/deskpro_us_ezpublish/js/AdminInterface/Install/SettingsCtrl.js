define(['DeskPRO/Util/Strings', 'DeskPRO/Util/Util'], function(Strings, Util) {
	return ['$scope', 'Api', '$q', '$modal', function($scope, Api, $q, $modal) {
		...
		function runTest(username, password) {
			...
			Api.sendPostJson('/apps/packages/deskpro_us_ezpublish/test-settings', postData).then(function(res) {
				deferred.resolve({
					log: res.data.log || '',
		...
		$scope.openTestModal = function(existing_results) {
			...
			var inst = $modal.open({
				templateUrl: 'deskpro_us_ezpublish/Install/test-settings-modal.html',
