 ...
 * @category Entities
 */
namespace deskpro_sendgrid\RequestHandler;
...
class PackageRequestHandler implements ApiPackageRequestHandlerInterface
{
    ...
    public function handleApiPackageRequest(ApiPackageRequestContext $context)
    {
        switch ($context->getAction()) {
            case 'get-url':
                return $this->getUrlAction($context);
            default:
     ...
     */
    public function getUrlAction(ApiPackageRequestContext $context)
    {
        $router = $context->getContainer()->getRouter();
        $url    = $router->generateUrl('email_callback');
