...
class DbalTicketDateLastUserReplyTermCompilerTest extends AbstractDbalTicketFilterTermCompilerTest
{
    ...
    public function testSimpleCases()
    {
        ...
        foreach ($ops as $op => $where) {
            if (TermInterface::OP_RANGE === $op || TermInterface::OP_NOT_RANGE === $op) {
                $term       = new TicketDateLastUserReplyTerm(array('date' => $date, 'date2' => $date2), $op);
                $query_part = $this->term_compiler->compile($term);
                $this->assertParameters($query_part, array('date' => $check1, 'date2' => $check2));
            } else {
                $term       = new TicketDateLastUserReplyTerm(array('date' => $date), $op);
                $query_part = $this->term_compiler->compile($term);
    ...
    public function testSingleDateIntoRange()
    {
        ...
        foreach ($ops as $op => $where) {
            $term       = new TicketDateLastUserReplyTerm(array('date' => $date, 'ignore_time' => true), $op);
            $query_part = $this->term_compiler->compile($term);
