    ...
    """
{
  ...
  "article": 1
    ...
    And the JSON node "data.task.id" should be equal to 1
    ...
    """
{
  ...
  Scenario: I GET all linked items
    ...
    And the JSON node "data" should exist
    And the JSON node "data[0].task.id" should be equal to 1
    And the JSON node "data[0].links.self" should be equal to "/api/v2/task_links/1"
  ...
  Scenario: I GET all linked items for a particular task
    ...
    And the response status code should be 200
    And the JSON node "data" should exist
    And the JSON node "data[0].article.id" should be equal to 1
