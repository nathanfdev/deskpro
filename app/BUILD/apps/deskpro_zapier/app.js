	...
	return {
		init: function() {
			if (this.getSetting('widget_ticket')) {
				this.registerWidgetTab('ticket', '@properties.tab', "Zapier ({{content.matches.length}})", 'tab.html', function($scope, $app, $ticket) {
					$scope.matches = [];
			...
			if (this.getSetting('widget_profile')) {
				this.registerWidgetTab('user', '@summary.tab', "Zapier ({{content.matches.length}})", 'tab.html', function($scope, $app, $person) {
					$scope.matches = [];
