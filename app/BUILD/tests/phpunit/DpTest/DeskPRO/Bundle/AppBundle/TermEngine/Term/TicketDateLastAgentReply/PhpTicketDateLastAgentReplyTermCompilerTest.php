...
class PhpTicketDateLastAgentReplyTermCompilerTest extends AbstractPhpTermCompilerTest
{
    ...
    public function testCompileIs()
    {
        ...
        $ticket->offsetGet('date_last_agent_reply')->willReturn($date);
        $this->assertTicketCheck(
            $php_check,
            true,
            $ticket
        );
        $ticket->offsetGet('date_last_agent_reply')->willReturn($date2);
        $this->assertTicketCheck(
            $php_check,
            false,
            $ticket
    ...
    public function testGreaterThan()
    {
        ...
        $ticket->offsetGet('date_last_agent_reply')->willReturn($tomorrow);
        $this->assertTicketCheck(
            $php_check,
            true,
            $ticket
        );
        $ticket->offsetGet('date_last_agent_reply')->willReturn($yesterday);
        $this->assertTicketCheck(
            $php_check,
            false,
            $ticket
    ...
    public function testInRange()
    {
        ...
        $ticket->offsetGet('date_last_agent_reply')->willReturn($today);
        $this->assertTicketCheck(
            $php_check,
            true,
            $ticket
        );
        $ticket->offsetGet('date_last_agent_reply')->willReturn($two_days_ago);
        $this->assertTicketCheck(
            $php_check,
            false,
            $ticket
