...
{
    /**
     * @var DbalTicketDateLastAgentReplyTermCompiler
     */
    ...
    public function testSimpleCases()
    {
        ...
        foreach ($ops as $op => $where) {
            if (TermInterface::OP_RANGE === $op || TermInterface::OP_NOT_RANGE === $op) {
                $term       = new TicketDateLastAgentReplyTerm(['date' => $date, 'date2' => $date2], $op);
                $query_part = $this->term_compiler->compile($term);
                $this->assertParameters($query_part, ['date' => $check1, 'date2' => $check2]);
            } else {
                $term       = new TicketDateLastAgentReplyTerm(['date' => $date], $op);
                $query_part = $this->term_compiler->compile($term);
    ...
    public function testSingleDateIntoRange()
    {
        ...
        foreach ($ops as $op => $where) {
            $term       = new TicketDateLastAgentReplyTerm(['date' => $date, 'ignore_time' => true], $op);
            $query_part = $this->term_compiler->compile($term);
