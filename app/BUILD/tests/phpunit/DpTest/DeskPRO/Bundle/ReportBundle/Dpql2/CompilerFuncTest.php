...
class CompilerFuncTest extends AbstractCompilerTest
{
    public function test_concat()
    {
        $this->assertDpqlQuery(
            <<<'DPQL'
SELECT DPQL_CONCAT(DPQL_FORMAT(tickets.urgency, 'number', 2), DPQL_MONTH(tickets.date_created), urgency), tickets.subject
FROM tickets
DPQL
            ,
            <<<'SQL'
...
DPQL
            ,
            <<<'SQL'
SELECT /*+ MAX_EXECUTION_TIME(30000) */ HOUR('2017-12-22 12:56:00')
FROM `tickets`
LIMIT 2500
SQL
        );
