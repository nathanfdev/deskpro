Feature: Widget Settings
  ...
  Scenario Outline: I change chat settings
    ...
    Then the response status code should be 200
    And the response should be in JSON
    And the JSON node "data.global.chat.email_validation" should be equal to "<email_validation>"
    And the JSON node "data.global.chat.require_login" should be equal to "<require_login>"
