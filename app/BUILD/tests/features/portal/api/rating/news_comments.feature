@new
Feature: /news_comments endpoint
  To rate news comments
  As an API user
  I want an endpoint to rate news comments

  Background:
    Given I'm authenticated as admin
    And no "News" records exist
    And only the following "NewsCategory" records exist:
      | #    | title                     |
      | csc1 | First News Category       |

  Scenario: I GET rate count of news comments
    Given only the following "News" records exist:
      | #        | slug      | title          | status    | category |
      | down1   | News1      | Test Comment1   | published | {csc1}   |
    And only the following "NewsComment" records exist:
      | #    | news     | person  | content      | is_reviewed |
      | com1 | {down1} | {admin} | comment1      | 1            |
    When I send a GET request to "/portal/api/news_comments/{com1}/rate_count"
    Then the response should be in JSON
    And the response status code should be 200
    And the JSON node "data" should exist
    And the JSON node "data[1].total" should exist


  Scenario: I upvote a news comment
    Given only the following "News" records exist:
      | #        | slug      | title          | status    | category |
      | down1   |   News1    | Test Comment    | published | {csc1}   |
    And only the following "NewsComment" records exist:
      | #    | news     | person  | content      | is_reviewed |
      | com1 | {down1}  | {admin} | comment1 | 1          |
    When I send a POST request to "/portal/api/news_comments/{com1}/rate" with body:
    """
{
  "upvote": 1
}
    """
    Then the response should be in JSON
    And the response status code should be 200
    And the JSON node "data.total_rating" should exist

    And the JSON node "data.total_rating" should be equal to "1"


  Scenario: I downvote a news comment
    Given only the following "News" records exist:
      | #        | slug      | title          | status    | category |
      | down1   | News1      | Test Comment1   | published | {csc1}   |
    And only the following "NewsComment" records exist:
      | #    | news     | person  | content      | is_reviewed |
      | com1 | {down1}  | {admin} | comment1 | 1          |
    When I send a POST request to "/portal/api/news_comments/{com1}/rate" with body:
    """
{
  "upvote": 0
}
    """
    Then the response should be in JSON
    And the response status code should be 200
    And the JSON node "data.total_rating" should exist

    And the JSON node "data.total_rating" should be equal to "-1"
