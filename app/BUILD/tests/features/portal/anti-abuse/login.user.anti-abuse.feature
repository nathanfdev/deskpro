Feature: Login anti-abuse feature
  ...
  Scenario: Checking lockout response for user
    ...
    And I set "login" rate limit to 5 attempt within 1 minute with "lockout" response and 15 minutes lockout time for agent
    And I set "login" rate limit to 5 attempt within 1 minute with "lockout" response and 15 minutes lockout time for guest
