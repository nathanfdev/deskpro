Feature: Login anti-abuse feature
  ...
  Scenario: Checking lockout response for agent/admin
    ...
    Given I set "login" rate limit to 5 attempts within 1 minute with "lockout" response and 15 minutes lockout time for guest
    Given I set "login" rate limit to 1 attempt within 1 minute with "lockout" response and 15 minutes lockout time for agent
  ...
  Scenario: Checking captcha response for admin/agent
    ...
    Given I set "login" rate limit to 1 attempt within 1 minute with "captcha" response for agent
