...
Feature: /{id}/ticket_slas endpoint
  ...
  Scenario: I create a ticket SLA
    When I send a POST request to "/api/v2/3/ticket_slas" with body:
"""
{
  "sla": 2,
  "sla_status": "warning"
  ...
  Scenario: I get created ticket SLA
    When I send a GET request to "/api/v2/3/ticket_slas?include=ticket,sla"
...
"""
{
  ...
  Scenario: I try update non-existing ticket SLA
    When I send a PUT request to "/api/v2/3/ticket_slas" with body:
"""
{
  "sla": 1,
  "sla_status": "ok"
...
"""
{
  ...
  Scenario: I try to get SLAs for non-existing ticket
    When I send a GET request to "/api/v2/3000/ticket_slas"
    Then the response status code should be 404
  ...
  Scenario: I try to get ticket SLA for non-existing parent SLA
    When I send a GET request to "/api/v2/3/ticket_slas/3000"
    Then the response status code should be 404
    And the JSON node "status" should be equal to 404
