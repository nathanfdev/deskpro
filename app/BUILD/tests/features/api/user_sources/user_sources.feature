Feature: /user_sources endpoint
  ...
  Scenario Outline: I get list of user sources
    ...
    And the JSON node "data[1].source_type" should be equal to "deskpro"
    And the JSON node "data[1].display_options" should have 0 elements
    And the JSON node "data[1].id" should exist
    And the JSON node "data[1].title" should be equal to "DeskPRO"
    And the JSON node "data[1].type" should be equal to <context>
    And the JSON node "data[1].display_order" should be equal to "-10"
