@new
Feature: /twilio_queues endpoint

  Background:
    Given I'm authenticated as admin

  Scenario: I retrieve a list of twilio queues
    Given only the following TwilioQueue records exist:
      | #  | Name    | Routing Model |
      | q1 | Queue 1 | round_robin   |
      | q2 | Queue 2 | round_robin   |
      | q3 | Queue 3 | round_robin   |

    When I send a GET request to "/api/v2/twilio_queues"
  ...
  Scenario: I create a new twilio queue
    Given only the following User records exist:
      ...
      | p2 | Person 2 |
      | p3 | Person 3 |
    When I send a POST request to "/api/v2/twilio_queues" with body:
    """
{
  "name": "My Queue",
    ...
    """
{
  ...
  Scenario: I delete twilio queue
    Given only the following TwilioQueue records exist:
      ...
      | q2 | Queue 2 | round_robin   |
      | q3 | Queue 3 | round_robin   |
    When I send a DELETE request to "/api/v2/twilio_queues/{q2}"
    Then the response status code should be 200
