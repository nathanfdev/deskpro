...
Feature: /tasks/{id}/subtasks endpoint
  ...
  Scenario: Successfully create a subtask
    When I send a POST request to "/api/v2/tasks/1/subtasks" with body:
    """
{
  "title": "My test subtask"
    ...
    And the header "Location" should be equal to "/api/v2/tasks/1/subtasks/1"
  ...
  Scenario: I GET a single subtask
    When I send a GET request to "/api/v2/tasks/1/subtasks/1"
    Then the response should be in JSON
  ...
  Scenario: I modify a subtask
    When I send a PUT request to "/api/v2/tasks/1/subtasks/1" with body:
    """
{
  "is_done": true
}
    ...
    """
{
  ...
  Scenario: I DELETE a single task
    When I send a DELETE request to "/api/v2/tasks/1/subtasks/1"
    Then the response should be in JSON
    And the response status code should be 200
