@new
Feature: /api/v2/community_forums/{id}/splash_image_upload
  To upload splash images to community forum
  ...
  Scenario: I upload a splash image
    When I send a POST request to "/api/v2/community_forums/{t1}/splash_image_upload" with "DummyImage"
    Then the response status code should be 200
    And the JSON node "data.image" should exist

  Scenario: I try to select a splash image for community forum
    When I send a POST request to "/api/v2/community_forums/{t1}/splash_image" with body:
    """
{
    "image": {
        "id": "GMgdkY1xfDA",
