...
Feature: /feedback/counts endpoint
  ...
  Scenario: I GET count of feedback grouped by category
    ...
    And the JSON node "data.nested[0].count" should be equal to 37
    And the JSON node "data.nested[0].title" should be equal to "Suggestion"

    And the JSON node "data.nested[1].id" should be equal to 2
    And the JSON node "data.nested[1].count" should be equal to 18
    And the JSON node "data.nested[1].title" should be equal to "Feature Request"

    And the JSON node "data.nested[2].id" should be equal to 3
    And the JSON node "data.nested[2].title" should be equal to "Bug Report"
    And the JSON node "data.nested[2].count" should be equal to 9
  ...
  Scenario: I GET count of feedback with status active grouped by status_category
    ...
    And the response status code should be 200
    And the JSON node "data" should exist
    And the JSON node "data.count" should be equal to 32
    And the JSON node "data.grouped_by" should be equal to "status_category"
    And the JSON node "data.nested[0].count" should be equal to 8
    And the JSON node "data.nested[0].title" should be equal to "Gathering Feedback"
