...
class BrandSettingContext extends BaseContext
{
    ...
    public function onlySettingWithNameAndValueForBrandExists($brand, $name, $value)
    {
        ...
        $this->dataContext->onlyTheFollowingRecordsExist('BrandSetting', new TableNode($table));
        $settings = $this->get('settings_resolver')->getBrandSettings($brand->getId(), true);
        $this->get('brand_stack')->getActive()->setSettings($settings);
    }
}
