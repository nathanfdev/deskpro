...
{
    $res = new Statement\Query(array(array('ANY', A)));

    $this->_result = $res;
}

start ::= select_paren(A) select_union(B) trailing_semicolon .
{
    $parts = array(array('ANY', A));
    if (B) {
        $parts = array_merge($parts, B);
    }
...
select_union(res) ::= select_union(A) OP_UNION select_paren(B) .
{
    if (!A) {
        res = array();
    } else {
...
select_union(res) ::= select_union(A) OP_UNION OP_DISTINCT select_paren(B) .
{
    if (!A) {
        res = array();
    } else {
...
{
	$q = new Statement\SelectPart(B, C);
...
from_clause(res) ::= FROM select_subquery_part(A) .
{
    res = A;
}
...
expression(res) ::= COLUMN(A) OP_IN select_subquery_part(B) .
{
    res = new Statement\Part\InSubquery(new Statement\Part\Column(explode('.', A)), B);
}

expression(res) ::= COLUMN(A) OP_NOT OP_IN select_subquery_part(B) .
{
    res = new Statement\Part\InSubquery(new Statement\Part\Column(explode('.', A)), B);
}
...
{
    res = new Statement\Part\Exists(A, B);
}
