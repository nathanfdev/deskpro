...
class CommunitySubscriptions extends AbstractJob
{
    ...
    public function run()
    {
        ...
        foreach ($brands as $brand) {
            ...
            if (!$brandSettingsResolver->getSetting(PortalSettingsResolver::SUBSCRIPTION_COMMUNITY)) {
                $brandStack->pop();
            ...
            $published = $this->getContainer()->getEm()->createQuery('
                ...
                WHERE ct.status IN (:statuses) AND ct.date_published > :date AND ct.brand = :brand_id
                ORDER BY ct.date_published DESC
            ')->setMaxResults(250)->execute(
                [
                    'date'     => $lastDate,
                    'statuses' => [
            ...
            $updated = $this->getContainer()->getEm()->createQuery('
                SELECT ct
                FROM DeskPRO:CommunityTopic ct INDEX BY ct.id
                WHERE ct.status IN (:statuses) AND (ct.date_updated > :date OR ct.date_last_comment > :date) AND ct.brand = :brand_id
            ...
            if (!$updated && !$published) {
                $this->logStatus('No new community topics for '.$brand.' brand');
                $brandStack->pop();
            ...
            $catGroups = $this->getContainer()->getDb()->fetchAllGrouped('
                SELECT community_forum_id, usergroup_id
                FROM community_forum2usergroup
            ', [], 'community_forum_id', null, 'usergroup_id');
            ...
            foreach ($userToTopic as $personId => $topics) {
                ...
                if ($this->getContainer()->get('deskpro.feature_flags')->hasBeta('email_templates')) {
                    $viewModel = $this->getContainer()->get('email.user_viewmodel_factory')
                        ->createCommunityTopicSubscriptionModel($updatedItems);
            ...
            if ($userToTopic) {
                $this->logStatus('Sent '.count($userToTopic).' notifications for '.$brand.' brand');
            }
