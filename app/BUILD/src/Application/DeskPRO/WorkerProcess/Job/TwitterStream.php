...
class TwitterStream extends AbstractJob
{
    ...
    protected function processStatus(TwitterAccount $account, $data)
    {
        ...
        } elseif (!empty($data->retweeted_status) && $data->retweeted_status->user->id_str == $account->getUserId()) {
            $account_status->status_type = 'retweet';
        } elseif (!empty($data->in_reply_to_user_id_str) && $data->in_reply_to_user_id_str == $account->getUserId()) {
