    ...
    public function quickSearch($q)
    {
        ...
        try {
            ...
            if (!empty($fragment)) {
                ...
                foreach ($matches[1] as $ticketId) {
                    $ticket = $this->em->getRepository('DeskPRO:Ticket')->findTicketId((int) $ticketId);
            ...
            if (is_int($index) && array_key_exists($index + 1, $path)) {
                $ticket = $this->em->getRepository('DeskPRO:Ticket')->findTicketId((int)$path[$index + 1]);
                if ($ticket && $this->person->PermissionsManager->TicketChecker->canView($ticket)) {
                    $results['ticket'][] = $ticket;
                }
            }
