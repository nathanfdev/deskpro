...
class ContentInitializer extends BaseContentInitializer
{
    ...
    {
        App::getDb()->exec("DELETE FROM content_search WHERE object_type IN ('article','download','community','news','topic')");
        App::getDb()->exec("DELETE FROM content_search_attribute WHERE object_type IN ('article','download','community','news','topic')");
    }
}
