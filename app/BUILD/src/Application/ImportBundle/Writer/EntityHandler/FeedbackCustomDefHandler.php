...
class FeedbackCustomDefHandler extends AbstractCustomDefHandler
{
    ...
    {
        if (!$model instanceof Model\FeedbackCustomDef) {
    ...
    private function findOrCreateCustomDef($entity_id)
    {
        if ($entity_id) {
            ...
            if ($custom_def) {
                $this->logger->debug(sprintf('Found existing feedback custom def, id=%s', $entity_id));

                return $custom_def;
            }
        }

        $this->logger->debug('Creating a new feedback custom def');
