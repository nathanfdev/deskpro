...
/*db:default*/ CREATE TABLE voice_queues (id INT AUTO_INCREMENT NOT NULL, account_id INT DEFAULT NULL, name VARCHAR(255) NOT NULL, task_queue_sid VARCHAR(100) NOT NULL, routing_model VARCHAR(255) NOT NULL, max_queue_size INT NOT NULL, greetAsset_id INT DEFAULT NULL, loopAsset_id INT DEFAULT NULL, voicemailAsset_id INT DEFAULT NULL, INDEX IDX_80C86EA9B6B5FBA (account_id), UNIQUE INDEX UNIQ_80C86EA96094606 (greetAsset_id), UNIQUE INDEX UNIQ_80C86EADC60BD4E (loopAsset_id), UNIQUE INDEX UNIQ_80C86EA43642E22 (voicemailAsset_id), UNIQUE INDEX task_queue_sid (task_queue_sid), UNIQUE INDEX name (name), PRIMARY KEY(id)) DEFAULT CHARACTER SET utf8 COLLATE utf8_unicode_ci ENGINE = InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci;
...
/*db:default*/ ALTER TABLE voice_queues ADD CONSTRAINT FK_80C86EA9B6B5FBA FOREIGN KEY (account_id) REFERENCES voice_accounts (id) ON DELETE CASCADE;
...
/*db:default*/ ALTER TABLE voice_accounts ADD twiml_app_sid VARCHAR(100) DEFAULT NULL;
/*db:default*/ ALTER TABLE agent_data ADD available_status VARCHAR(100) NOT NULL;
/*db:default*/ ALTER TABLE agent_data ADD agent_calls_enabled TINYINT(1) NOT NULL;
/*db:default*/ CREATE TABLE voice_tickets (ticket_id INT NOT NULL, voice_id INT NOT NULL, INDEX IDX_D326667700047D2 (ticket_id), INDEX IDX_D3266671672336E (voice_id), PRIMARY KEY(ticket_id, voice_id)) DEFAULT CHARACTER SET utf8 COLLATE utf8_unicode_ci ENGINE = InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci;
/*db:default*/ ALTER TABLE voice_tickets ADD CONSTRAINT FK_D326667700047D2 FOREIGN KEY (ticket_id) REFERENCES tickets (id) ON DELETE CASCADE;
/*db:default*/ ALTER TABLE voice_tickets ADD CONSTRAINT FK_D3266671672336E FOREIGN KEY (voice_id) REFERENCES voice_phone_calls (id) ON DELETE CASCADE;
/*db:default*/ CREATE TABLE voice_phone_calls (id INT AUTO_INCREMENT NOT NULL, number_id INT DEFAULT NULL, call_sid VARCHAR(50) NOT NULL, conference_sid VARCHAR(50) DEFAULT NULL, from_number VARCHAR(50) NOT NULL, data LONGTEXT NOT NULL COMMENT '(DC2Type:json_array)', INDEX IDX_6679AE4C30A1DE10 (number_id), UNIQUE INDEX call_sid (call_sid), UNIQUE INDEX conference_sid (conference_sid), PRIMARY KEY(id)) DEFAULT CHARACTER SET utf8 COLLATE utf8_unicode_ci ENGINE = InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci;
/*db:default*/ ALTER TABLE voice_phone_calls ADD CONSTRAINT FK_6679AE4C30A1DE10 FOREIGN KEY (number_id) REFERENCES voice_numbers (id) ON DELETE CASCADE;
/*db:default*/ ALTER TABLE voice_phone_calls ADD task_sid VARCHAR(50) DEFAULT NULL;
/*db:default*/ CREATE TABLE voice_phone_call_participants (id INT AUTO_INCREMENT NOT NULL, phone_call_id INT DEFAULT NULL, person_id INT DEFAULT NULL, call_sid VARCHAR(50) NOT NULL, INDEX IDX_EBEABFDFC0EA171E (phone_call_id), INDEX IDX_EBEABFDF217BBB47 (person_id), PRIMARY KEY(id)) DEFAULT CHARACTER SET utf8 COLLATE utf8_unicode_ci ENGINE = InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci;
/*db:default*/ ALTER TABLE voice_phone_call_participants ADD CONSTRAINT FK_EBEABFDFC0EA171E FOREIGN KEY (phone_call_id) REFERENCES voice_phone_calls (id) ON DELETE CASCADE;
/*db:default*/ ALTER TABLE voice_phone_call_participants ADD CONSTRAINT FK_EBEABFDF217BBB47 FOREIGN KEY (person_id) REFERENCES people (id) ON DELETE CASCADE;
/*db:default*/ ALTER TABLE voice_phone_calls ADD status VARCHAR(50) NOT NULL;
SQL
        );
    }
}
