...
class TriggerTermConverter
{
    ...
    private function upgradeTerm_agent($type, $op, OptionsArray $options)
    {
        ...
        return new Terms\CheckAgent($op, ['agent_ids' => $opt]);
    }
    ...
    private function upgradeTerm_agent_team($type, $op, OptionsArray $options)
    {
        ...
        return new Terms\CheckAgentTeam($op, ['team_ids' => $opt]);
    }

    private function upgradeTerm_api_key($type, $op, OptionsArray $options)
    {
        return new Terms\CheckApiKey($op, ['api_key_id' => $options->get('api_key', 0)]);
    }
    ...
    private function upgradeTerm_category($type, $op, OptionsArray $options)
    {
        ...
        return new Terms\CheckCategory($op, ['category_ids' => $opt]);
    }

    private function upgradeTerm_creation_system($type, $op, OptionsArray $options)
    {
        return new Terms\CheckCreationSystem($op, ['creation_system' => $options->get('creation_system')]);
    }

    private function upgradeTerm_creation_system_option($type, $op, OptionsArray $options)
    {
        return new Terms\CheckCreationSystemOption($op, ['creation_system_option' => $options->get('website_url')]);
    }
    ...
    private function upgradeTerm_current_day($type, $op, OptionsArray $options)
    {
        ...
        return new Terms\CheckDayOfWeek($op, ['days' => $opt, 'tz' => 'UTC', 'var' => 'now']);
    }
    ...
    private function upgradeTerm_day_created($type, $op, OptionsArray $options)
    {
        ...
        return new Terms\CheckDayOfWeek($op, ['days' => $opt, 'tz' => 'UTC', 'var' => 'date_created']);
    }
    ...
    private function upgradeTerm_department($type, $op, OptionsArray $options)
    {
        ...
        return new Terms\CheckDepartment($op, ['department_ids' => $opt]);
    }
    ...
    {
        return new Terms\CheckEmailBody($op, ['body' => $options->get('message', 'NO MESSAGE')]);
    }

    private function upgradeTerm_email_cc_email($type, $op, OptionsArray $options)
    {
        return new Terms\CheckEmailCcAddress($op, ['email' => $options->get('email_address', 'NO EMAIL')]);
    }

    private function upgradeTerm_email_cc_name($type, $op, OptionsArray $options)
    {
        return new Terms\CheckEmailCcName($op, ['name' => $options->get('name', 'NO NAME')]);
    }

    private function upgradeTerm_email_from_email($type, $op, OptionsArray $options)
    {
        return new Terms\CheckEmailFromAddress($op, ['email' => $options->get('email_address', 'NO EMAIL')]);
    }

    private function upgradeTerm_email_from_name($type, $op, OptionsArray $options)
    {
        return new Terms\CheckEmailFromName($op, ['name' => $options->get('email_name', 'NO NAME')]);
    }

    private function upgradeTerm_email_subject($type, $op, OptionsArray $options)
    {
        return new Terms\CheckEmailSubject($op, ['subject' => $options->get('subject', 'NO SUBJECT')]);
    }

    private function upgradeTerm_email_to_email($type, $op, OptionsArray $options)
    {
        return new Terms\CheckEmailToAddress($op, ['email' => $options->get('email_address', 'NO EMAIL')]);
    }

    private function upgradeTerm_email_to_name($type, $op, OptionsArray $options)
    {
        return new Terms\CheckEmailToName($op, ['name' => $options->get('name', 'NO NAME')]);
    }

    private function upgradeTerm_email_header($type, $op, OptionsArray $options)
    {
        return new Terms\CheckEmailHeader($op, [
    ...
    private function upgradeTerm_feedback_rating($type, $op, OptionsArray $options)
    {
        ...
        return new Terms\CheckSatisfaction($op, ['rating' => $r]);
    }
    ...
    private function upgradeTerm_gateway_account($type, $op, OptionsArray $options)
    {
        ...
        return new Terms\CheckEmailAccount($op, ['email_account_ids' => [$options->get('gateway_account', 0)]]);
    }
    ...
    private function upgradeTerm_label($type, $op, OptionsArray $options)
    {
        ...
        return new Terms\CheckLabel($op, ['labels' => $labels]);
    }
    ...
    private function upgradeTerm_message($type, $op, OptionsArray $options, $event_trigger)
    {
        ...
        if (strpos($event_trigger, 'agent') !== false) {
            return new Terms\CheckAgentMessage($op, ['message' => $message]);
        } else {
            return new Terms\CheckUserMessage($op, ['message' => $message]);
        }
    }
    ...
    {
        return new Terms\CheckOrgEmailDomain($op, ['email_domain' => $options->get('email_domain', 'NO DOMAIN')]);
    }
    ...
    private function upgradeTerm_org_label($type, $op, OptionsArray $options)
    {
        ...
        return new Terms\CheckOrgLabel($op, ['labels' => $labels]);
    }
    ...
    {
        return new Terms\CheckOrgName($op, ['name' => $options->get('name', 'NO NAME')]);
    }

    private function upgradeTerm_person_email($type, $op, OptionsArray $options)
    {
        return new Terms\CheckUserEmail($op, ['email' => $options->get('email', 'NO EMAIL')]);
    }
    ...
    private function upgradeTerm_person_email_domain($type, $op, OptionsArray $options)
    {
        ...
        return new Terms\CheckUserEmail($op, ['email' => $regex]);
    }
    ...
    private function upgradeTerm_person_field($type, $op, OptionsArray $options)
    {
        ...
        return new Terms\CheckUserField($op, [
            'field_id' => $field_id,
            'value'    => $value,
        ]);
    }
    ...
    {
        return new Terms\CheckPerformer($op, [
    ...
    {
        return new Terms\CheckUserName($op, [
    ...
    private function upgradeTerm_person_label($type, $op, OptionsArray $options)
    {
        ...
        return new Terms\CheckUserLabel($op, ['labels' => $labels]);
    }
    ...
    private function upgradeTerm_person_usergroup($type, $op, OptionsArray $options)
    {
        ...
        return new Terms\CheckUserUsergroups($op, ['usergroup_ids' => $opt]);
    }
    ...
    private function upgradeTerm_priority($type, $op, OptionsArray $options)
    {
        ...
        return new Terms\CheckPriority($op, ['priority_ids' => $opt]);
    }
    ...
    private function upgradeTerm_product($type, $op, OptionsArray $options)
    {
        ...
        return new Terms\CheckProduct($op, ['product_ids' => $opt]);
    }

    private function upgradeTerm_sla($type, $op, OptionsArray $options)
    {
        return new Terms\CheckSla($op, ['sla_ids' => $this->getSingleArrayValue($options, 'sla_id')]);
    }
    ...
    private function upgradeTerm_sla_status($type, $op, OptionsArray $options)
    {
        ...
        return new Terms\CheckSlaStatus($op, [
    ...
    {
        return new Terms\CheckStatus($op, ['status' => $options->get('status', null)]);
    }

    private function upgradeTerm_subject($type, $op, OptionsArray $options)
    {
        return new Terms\CheckSubject($op, ['subject' => $options->get('subject', 'NO SUBJECT')]);
    }
    ...
    private function upgradeTerm_ticket_field($type, $op, OptionsArray $options)
    {
        ...
        return new Terms\CheckTicketField($op, [
            'field_id' => $field_id,
            'value'    => $value,
        ]);
    }
    ...
    {
        return new Terms\CheckTimeOfDay($op, [
    ...
    {
        return new Terms\CheckDateCreated($op, [
    ...
    {
        return new Terms\CheckUrgency($op, ['urgency1' => $options->get('num', 1)]);
    }

    private function upgradeTerm_user_performer_email($type, $op, OptionsArray $options)
    {
        return new Terms\CheckPerformerEmail($op, ['email' => $options->get('user_email')]);
    }
    ...
    private function upgradeTerm_workflow($type, $op, OptionsArray $options)
    {
        ...
        return new Terms\CheckWorkflow($op, ['workflow_ids' => $opt]);
    }
}
