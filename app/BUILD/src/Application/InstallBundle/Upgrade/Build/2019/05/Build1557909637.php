...
class Build1557909637 extends AbstractBuild implements OnlineBuildInterface, SkipPostBuildInterface
{
    public function addNewTables()
    {
        $this->out('Creating custom_def_download');
    ...
    public function runAlters()
    {
        if (defined('DPC_IS_CLOUD')) {
            // on cloud just swap the tables, can copy data later
            $this->out('Re-create page_view_log');
            $this->execDbQuery('default', 'CREATE TABLE page_view_log_new (id INT AUTO_INCREMENT NOT NULL, object_type INT NOT NULL, object_id INT NOT NULL, view_action INT NOT NULL, person_id INT DEFAULT NULL, meta LONGTEXT DEFAULT NULL COMMENT \'(DC2Type:json_array)\', date_created DATETIME NOT NULL, INDEX object_idx (object_type, object_id), INDEX date_created_idx (date_created), PRIMARY KEY(id)) DEFAULT CHARACTER SET utf8 COLLATE utf8_unicode_ci ENGINE = InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci');
            $this->out('Moving old table');
            $this->execDbQuery('default', 'RENAME TABLE page_view_log TO page_view_log_1557909637, page_view_log_new TO page_view_log');
        } else {
            $this->out('Adding meta to page_view_log');
