...
/**
 * Takes a FQL query and compiles into plain text useful for visual/debugging.
 */
class DebugCompiler
{
    /**
    ...
    private function walkGroup(array $boolPart)
    {
        ...
        if ($boolPart['operator'] === 'NOT') {
            // SHOULD only be a single expr here, user would sub-nest another bool to get inner checks
            return "IF NOT (\n{$this->indent(1)}".implode("\n{$this->indent()}OR ", $expr)."\n{$this->indent(1)})";
        }

        return "IF (\n{$this->indent(1)}".implode("\n{$this->indent(1)}{$boolPart['operator']} ", $expr)."\n{$this->indent()})";
    }
