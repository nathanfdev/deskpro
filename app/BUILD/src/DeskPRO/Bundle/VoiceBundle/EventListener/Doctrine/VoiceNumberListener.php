...
class VoiceNumberListener implements EventSubscriber
{
    ...
    public function onCreate(VoiceNumber $number, LifecycleEventArgs $args)
    {
        ...
        if ($account instanceof TwilioVoiceAccount) {
            ...
            $this->container->get('twilio_adapter')->setTwimlAppId($number);
        } elseif ($account instanceof PlivoVoiceAccount) {
            // link number to the DeskPRO app
            $this->container->get('plivo_adapter')->setApplicationId($number);
        }
    ...
    public function onRemove(VoiceNumber $number, LifecycleEventArgs $args)
    {
        ...
        if ($account instanceof TwilioVoiceAccount) {
            ...
            try {
                $this->container->get('twilio_adapter')->unsetTwimlAppId($number);
