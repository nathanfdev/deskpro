...
class ColumnNode extends \Twig_Node
{
    public function compile(\Twig_Compiler $compiler)
    {
        ...
        if ($this->hasNode('className')) {
            $compiler->write("\$className .= '".addslashes($this->getNode('className')->getAttribute('value'))." ';\n");
        }
        if ($this->hasNode('small')) {
            $compiler->write("\$className .= 'small-".addslashes($this->getNode('small')->getAttribute('value'))." ';\n");
        }
        if ($this->hasNode('large')) {
            $compiler->write("\$className .= 'large-".addslashes($this->getNode('large')->getAttribute('value'))." ';\n");
        }
