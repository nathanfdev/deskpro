...
/**
 * Handler that wraps around DPQL_MINUTE() to provide a group fill.
 */
class DpqlMinute extends AbstractDpqlFunc
{
    /**
     * {@inheritdoc}
     */
    public function prepare(array $arguments, SelectPart $statement, $section, array $stack, SqlSelect $select, ResultMetadata $metadata)
    {
        if (count($arguments) != 1) {
            throw new DpqlException('DPQL_MINUTE() can only accept 1 argument.');
        }
        ...
        $res = new Prepared($sql, 'DPQL_MINUTE('.$prepped->name().')', false, 'numberraw');
