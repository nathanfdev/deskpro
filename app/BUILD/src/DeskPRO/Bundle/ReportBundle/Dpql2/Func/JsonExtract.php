 ...
 * Class JsonExtract.
 */
class JsonExtract extends AbstractFunc
{
    /**
     * {@inheritdoc}
     */
    public function prepare(array $arguments, SelectPart $statement, $section, array $stack, SqlSelect $select, ResultMetadata $metadata)
    {
        if (count($arguments) !== 2) {
            throw new DpqlException('JSON_EXTRACT() can only accept at least 2 arguments.');
        }
        if ($section !== 'select') {
            throw new DpqlException('JSON_EXTRACT() may only be used in SELECT.');
        }
