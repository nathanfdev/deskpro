...
class Hierarchy extends AbstractFunc
{
    ...
    public function prepare(array $arguments, SelectPart $statement, $section, array $stack, SqlSelect $select, ResultMetadata $metadata)
    {
        ...
        if (!in_array(count($arguments), [2, 3])) {
            throw new DpqlException('HIERARCHY() can only accept 2 or 3 arguments.');
        }
        ...
        if (array_key_exists(1, $arguments)) {
            if (!$arguments[1] instanceof Number) {
                throw new DpqlException('HIERARCHY() 2nd argument must be a number.');
            }
            $minDepth = $arguments[1]->getValue();
            if ($minDepth <= 0) {
                throw new DpqlException('HIERARCHY() 2nd argument must be a positive number.');
            }
        }
        ...
        if (array_key_exists(2, $arguments)) {
            if (!$arguments[2] instanceof Number) {
                throw new DpqlException('HIERARCHY() 3rd argument must be a number.');
            }
            $maxDepth = $arguments[2]->getValue();
            if ($maxDepth <= 0) {
                throw new DpqlException('HIERARCHY() 3rd argument must be a positive number.');
            }
        }
        ...
        if ($minDepth > $maxDepth) {
            throw new DpqlException('HIERARCHY() 3rd argument must be bigger or equal than the 2nd.');
        }
