 ...
 */
class DpqlX extends AbstractDpqlFunc
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        if ($section != 'group') {
            throw new DpqlException('DPQL_X() may only be used in GROUP BY.');
        }
        if (count($stack) > 1) {
            // note: the top of the stack is this function
            throw new DpqlException('DPQL_X() may only be used at the top-level.');
        }
