...
class TicketWhWaitingTimeRecalculateCommand extends ContainerAwareCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $numTickets     = $input->getOption('batch-size') ? $input->getOption('batch-size') : ($isBatchMode ? 1000 : 1);
        $minTicketId    = max($input->getOption('min-ticket-id') ? $input->getOption('min-ticket-id') : 0, 0);
