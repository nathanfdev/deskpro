...
class ActionAlertsListener implements EventSubscriberInterface
{
    ...
    public function onResponse(FilterResponseEvent $event)
    {
        ...
        if (($lastId = $request->headers->get('x-deskpro-return-actionalerts')) && $this->isPollingEnabled()) {
            ...
            if ($alerts) {
                $givenSplitPoint     = $request->headers->get('x-deskpro-return-actionalerts-splitpoint');
                $splitPoint          = $givenSplitPoint ?: '--action-alerts-'.Strings::random(30, Strings::CHARS_ALPHANUM);
