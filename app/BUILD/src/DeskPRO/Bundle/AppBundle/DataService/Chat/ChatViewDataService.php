...
class ChatViewDataService extends AbstractDataService
{
    public function getCustomDataForChat(ChatConversation $chat)
    {
        ...
        foreach ($this->getEnabledUserChatFields() as $chat_def) {
            if ($data = $chat->getCustomDataForField($chat_def)) {
                $val = CustomFieldUtil::getValueForCustomFormField($chat_def, $data);
            } else {
                $val = '';
            }

            $view[] = [
                'label' => $chat_def->getTitle(),
                'value' => $val,
            ];
        }
    ...
    protected function getEnabledUserChatFields()
    {
        ...
        $repo = $this->em->getRepository('DeskPRO:CustomDefChat');
