...
use DeskPRO\Bundle\AppBundle\Dpql2\Plugin\PluginInterface;
...
use DeskPRO\Bundle\AppBundle\Dpql2\Statement\Query;
    ...
    public function afterQuery(array $results)
    {
        ...
        if ($this->hierarchyDescendsFromId && count($results)) {
            ...
            while ($parentsLimit > 0) {
                $entries = $this->connection->executeQuery($sql, [$parentId])->fetchAll(\PDO::FETCH_NUM);
                if (count($entries)) {
                    $path .= "{$entries[0][0]} > ";
