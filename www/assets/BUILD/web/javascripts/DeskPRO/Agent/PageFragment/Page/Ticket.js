...
DeskPRO.Agent.PageFragment.Page.Ticket = new Orb.Class({
  ...
  initializeProperties: function() {
    ...
    this.saveReplyPrepareAjax = null;
    this.saveReplyAjaxCnt = 0;
    if (DeskPRO_Window.$q) {
  ...
  initPage: function(el) {
    ...
    // Becase of optimistic Reply save
    window.addEventListener('beforeunload', function (e) {
      if (self.saveReplyAjaxCnt > 0) {
  ...
  doHandleReplySave: function(ev, formData, handler, meta) {
    ...
    // Wait at least while previous prepare save reply completed
    if (this.saveReplyPrepareAjax) {
      return;
    }
    ...
    function saveReplySuccessCallback(result) {
      // we check self.changeManager sometimes because tab could be closed by this time
      ...
      if (result.client_messages) {
        ...
        // null out so handleTicketUpdate doesnt re-process them
      ...
      if (!result.close_tab && self.changeManager) {
    ...
    function saveReplyPrepareSuccessCallback(result) {
        if (result.client_messages) {
          DeskPRO_Window.getMessageChanneler().handleMessageAjax(result.client_messages);
    ...
    function sendSaveReplyRequest()
    {
        ...
        $.ajax({
          ...
          complete: function() {
            self.saveReplyAjaxCnt --;
            DeskPRO_Window.getMessageChanneler().poller.unpause();
            self.getReplyTextArea().data('disable-autosave', false);
          },
