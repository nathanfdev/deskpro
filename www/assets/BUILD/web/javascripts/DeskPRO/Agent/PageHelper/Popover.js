...
DeskPRO.Agent.PageHelper.Popover = new Orb.Class({
	...
	updatePositions: function() {
		...
		// Beside
		if (this.options.positionMode == 'side') {
			if (!DeskPRO_Window.paneVis.list || !DeskPRO_Window.paneVis.source || (DeskPRO_Window.paneVis.list && !DeskPRO_Window.paneVis.tabs)) {
				if (this.options.sidePosition == 'bottom') {
				...
				this.popoverOuter.css({
					'position': 'absolute',
					'z-index': 1800,
					'width': width+2+6-52, //2px for thi sborder, 6px for the popover border, 52 - sidebar menu offset
					'overflow': 'auto',
					'top': top ? top-3 : '',
					'left': 61,
					'bottom': bottom,
