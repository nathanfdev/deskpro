define ['Admin/Main/Ctrl/Base', 'DeskPRO/Util/Arrays'], (Admin_Ctrl_Base, Arrays) ->
  class Admin_ChatRoundRobin_Ctrl_Edit extends Admin_Ctrl_Base
    @CTRL_ID = 'Admin_ChatRoundRobin_Ctrl_Edit'
    @CTRL_AS = 'EditCtrl'
    ...
    init: ->
      ...
      @serviceDeps = @DataService.get 'ChatDeps'
    ...
    handleBulk: ->
      ...
      switch params[0]
        when 'd'
          @Api.sendGet("/chat_deps/#{params[1]}?with_agents_list=1").success( (data) =>
    ...
    showLogs: ->
      @Api.sendGet("/chat_round_robin/#{@robin.id}/logs").then (res) =>
        @$modal.open({
          template: res.data,
  ...
  Admin_ChatRoundRobin_Ctrl_Edit.EXPORT_CTRL()