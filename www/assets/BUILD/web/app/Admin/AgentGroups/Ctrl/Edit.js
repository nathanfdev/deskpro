define(['Admin/Main/Ctrl/Base', 'angular'], function(Admin_Ctrl_Base, angular) {
  class Admin_AgentGroups_Ctrl_Edit extends Admin_Ctrl_Base {
    ...
    changeAllPerms(type, section) {
      ...
      if (('perms' === type) && this.group.perms && this.group.perms[section]) {
        for (let perm of Object.keys(this.group.perms[section] || {})) {
    ...
    showDelete() {
      let inst;
      const deleteGroup = () => {
        return this.service.groups.remove(this.group).then(() => {
          // force reload deps perms
          // todo move deps perms as @group attribute
          this.$q.all([this.service.ticketDeps.all(true), this.service.chatDeps.all(true)]);
    ...
    showCopySettings() {
      ...
      const copySettings = settings => {
        ...
        return this.service.groups.get(settings.group_id).then(group => {
