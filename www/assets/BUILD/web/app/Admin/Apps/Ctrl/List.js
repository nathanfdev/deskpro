  ...
  class Admin_Apps_Ctrl_List extends Admin_Ctrl_Base {
    ...
    init() {
      ...
      this.$scope.packagesV2Filter = hide_installed =>
        function(pkg) {
          ...
          for (let instance of Array.from(ctrl.apps_v2)) {
    ...
    initialLoad() {
      ...
      apps2Promise.then( result => {
        return result.data.data.forEach(instance => {
      ...
      apps2PackagesPromise.then( result => {
        return result.data.data.forEach(app => {
    ...
    addAppInstance(instanceInfo, isNew) {
      ...
      if (isNew) {
        ...
        return (() => {
          const result = [];
          for (let p of Array.from(this.packages)) {
    ...
    removeAppInstance(instanceId, isNew) {
      ...
      if (isNew) {
        ...
        if (app) {
          ...
          this.apps.map(function(x) { if (x.package.name === app.package.name) { return hasOtherApp = true; } });
          if (!hasOtherApp) {
    ...
    showNewApp() {
      const saveNewApp = options => {
        ...
        return this.Api.sendPutJson('/apps/custom', postData).success( info => {
          return this.$state.go('apps.apps.custom_instance', {custom_id: `custom_${info.id}`});
        });
      };
        ...
        controller: ['$scope', '$modalInstance', function($scope, $modalInstance) {
          ...
          return $scope.startUpload = function() {
            ...
            if ($scope.form.upload_type === 'upload') {
              ...
              $scope.form.is_active = true;
              return me.Api.sendPost('/apps/upload-package', { file_url: $scope.form.upload_url }).then( function(result) {
                $scope.form.is_active = false;
