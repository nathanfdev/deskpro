  ...
  class Admin_TicketDeps_Ctrl_Edit extends Admin_Ctrl_Base {
    ...
    initialLoad() {
      const promise1 = this.depData.getEditDepartmentData(this.depId || null).then((data) => {
        ...
        if (this.depId) {
          this.layout_info.default = this.layout_info.default.filter(x => x.id !== this.depId);
          this.layout_info.custom = this.layout_info.custom.filter(x => x.id !== this.depId);
        }
        ...
        for (const name of ['link', 'win', 'embed', 'phpapi']) {
    ...
    successSaving() {
      return this.stopSpinner('saving_dep').then(() => this.Growl.success(this.getRegisteredMessage('saved_dep'), () => this.$state.go('tickets.ticket_deps.edit', { id: this.dep.id })));
    }
    ...
    changeAllPerms(group, perm) {
      ...
      if (group === 'agent') {
        for (id of Object.keys(this.form.agent_perms.groups || {})) {
          group = this.form.agent_perms.groups[id];
          if (!group.perms[perm].locked && (group.model.sys_name !== 'agent_all_perms') && (group.model.sys_name !== 'agent_all_safe_perms')) { group.perms[perm].state = _perm; }
        }
        return (() => {
          const result = [];
          for (id of Object.keys(this.form.agent_perms.agents || {})) {
