  ...
  class Admin_ApiKeys_Ctrl_LogsView extends Admin_Ctrl_Base {
    ...
    initialLoad() {
      return this.service.get(this.$stateParams.id).then((model) => {
        ...
        return this.service.loadLog(this.$stateParams.id).then(model => this.model = model);
      });
    }
    ...
    replay() {
      return this.service.replay(this.model, 'subrequest').then(model => this.$modal.open({
