define [
  ...
  class Admin_ApiKeys_Ctrl_Edit extends Admin_Ctrl_Base
    ...
    init: ->
      @agents = []
      @form = {isSuperUser: false, flags: [], apiVersions: 'api_v1,api_v2'}
    ...
    initialLoad: ->
      ...
      p1 = @service.keys.get(@$stateParams.id || null).then (model) =>
        ...
        apiVersions = [];
        if @form.flags.indexOf('api_v1') > -1
          apiVersions.push 'api_v1'
        if @form.flags.indexOf('api_v2') > -1
          apiVersions.push 'api_v2'
        @form.apiVersions = if apiVersions.length then apiVersions.join(',') else 'api_v1,api_v2'
    ...
    saveForm: ->
      ...
      if @form.apiVersions.includes('api_v1')
        @form.flags.push 'api_v1'
      if @form.apiVersions.includes('api_v2')
        @form.flags.push 'api_v2'
