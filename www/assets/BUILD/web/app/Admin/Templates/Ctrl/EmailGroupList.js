  ...
  class Admin_Templates_Ctrl_EmailGroupList extends Admin_Ctrl_Base {
    ...
    loadLegacyTemplates() {
      return this.Api2.sendGet('/email_templates/legacy_templates').then(
        res => {
          return this.$scope.toUpgradeTemplates = res.data;
      });
    }
    ...
    revertTemplate(id, triggers) {
      ...
      return this.showConfirm(message).result.then(() => {
        return this.Api2.sendGet(`/email_templates/revert_legacy_template/${id}`).then(
          () => {
            return this.loadLegacyTemplates();
        });
      });
    }
    ...
    deleteTemplate(id, triggers) {
      ...
      return this.showConfirm(message).result.then(() => {
        return this.Api2.sendDelete(`/email_templates/legacy_template/${id}`).then(
          () => {
            return this.loadLegacyTemplates();
        });
      });
