define(['Admin/Main/Ctrl/Base', 'DeskPRO/Util/Strings'], function(Admin_Ctrl_Base, Strings) {
  class Admin_Templates_Ctrl_EmailTemplateEditor extends Admin_Ctrl_Base {
    ...
    initialLoad() {
      let p;
      if (!this.$scope.is_new_email) {
        p = this.Api.sendGet(`/templates/${this.templateName}`).success(data => this.initTemplateData(data));
      ...
      p = this.Api.sendGet('/email-templates-info').success(data => this.initCustomNames(data.list.custom.groups.custom.templates));
      return p;
    }
