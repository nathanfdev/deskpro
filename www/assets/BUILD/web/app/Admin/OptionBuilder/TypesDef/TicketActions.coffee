  ...
  class Admin_OptionBuilder_TypesDef_TicketFilter extends BaseActionTypesDef
    ...
    loadDataOptions: ->
      if not @loadDataPromise
        ...
        promises = [apiV1]
        promises.push @Api2.sendGet('/ticket_statuses')

        if (window.DP_HAS_NEW_EMAILS)
          promises.push @Api2.sendGet('/email_templates/info')
        ...
        @$q.all(promises).then( (result) =>
          ...
          if @options_data?.ticket_fields
            for f in @options_data.ticket_fields
              @initFieldGetter 'SetTicketField', f, true
          if @options_data?.contextual_fields
            for f in @options_data.contextual_fields
              @initFieldGetter 'SetTicketContextualField', f, true
          if @options_data?.user_fields
            for f in @options_data.user_fields
    ...
    getSetStatus: (options = {}) ->
      ...
      options.options = @options_data.ticket_statuses.filter (status) -> !isNaN(status.id)
