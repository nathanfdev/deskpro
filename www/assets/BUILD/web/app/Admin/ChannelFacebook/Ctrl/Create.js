define([
  ...
  class Admin_ChannelFacebook_Ctrl_Create extends Admin_Ctrl_Base {
    ...
    init() {
      ...
      return require(['facebook'], FB => this.FB = FB
    ...
    selectPage(page_id) {
      ...
      for (const page of Array.from(this.available_user_pages)) {
        if (page.id === page_id) {
          selected_page = page;
          break;
        }
    ...
    _getPages() {
      ...
      start().then(() => FB.api(`/${this.app_id}`, 'GET', {}, (res) => {
      ...
      , (error) => {
        this.Growl.error(this.getRegisteredMessage('connected_fail'));
        return this._stopSpinnerTimeout('connecting_app');
      }).then(() => FB.api(`/${this.user_graph_id}/permissions`, 'GET', {}, (perms) => {
        const pp = (Array.from(perms.data).map(p => p.permission));
        if (Array.from(pp).includes('manage_pages') && Array.from(pp).includes('read_page_mailboxes')) {
        ...
        this.Growl.error(this.getRegisteredMessage('invalid_permissions'));
        return this._stopSpinnerTimeout('connecting_app');
      })).then(() => FB.api(`/${this.user_graph_id}/accounts`, 'GET', {}, (pages) => {
        ...
        const promises = [];
        for (const page of Array.from(pages.data)) {
    ...
    _stopSpinnerTimeout(name) {
      return this.$timeout(() => this.stopSpinner(name, true)
      , 0);
    }
