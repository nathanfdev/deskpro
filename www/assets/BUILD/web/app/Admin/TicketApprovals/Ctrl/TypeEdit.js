  ...
  class Admin_TicketApprovals_Ctrl_TypeEdit extends Admin_Ctrl_Base {
    ...
    init() {
      ...
      this.typeId = (this.$stateParams.id)
        ? parseInt(this.$stateParams.id.replace(/^type-(\d+)$/, '$1'), 10)
        : null;
    ...
    initialLoad() {
      ...
      if (this.typeId) {
        const promise = this.dataService.loadApprovalTypes(this.typeId)
          .then((data) => {
    ...
    saveForm() {
      // Growl messages
      const msgSuccess = this.getRegisteredMessage('approval_type_save_success');
    ...
    deleteType(id) {
      // Growl messages
      const msgSuccess = this.getRegisteredMessage('approval_type_delete_success');
      const msgFailure = this.getRegisteredMessage('approval_type_delete_failure');
