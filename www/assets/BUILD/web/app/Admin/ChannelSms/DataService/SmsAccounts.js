...
define([
  ...
  let Admin_ChannelSms_DataService_SmsAccounts;
  return (Admin_ChannelSms_DataService_SmsAccounts = class Admin_ChannelSms_DataService_SmsAccounts extends Admin_Main_DataService_Base {
  ...
  * @return {Promise}
    */
    loadList(reload) {
      ...
      const http_def = this.Api.sendGet('/channel/sms/accounts').success((data, status, headers, config) => {
        this._setListData(data.sms_accounts);
    ...
    _setListData(raw_recs) {
      return (() => {
        ...
        for (let rec of Array.from(raw_recs)) {
          const model = this.em.createEntity('sms_account', 'id', rec);
          model.retain();
  ...
  */
    updateModel(account) {
      const new_model = this.em.createEntity('sms_account', 'id', account);
