define([
  ...
  class UserFields extends BaseListEdit {
    static initClass() {
      this.$inject = ['Api', '$q'];
    }
    ...
    _doLoadList() {
      ...
      this.Api.sendDataGet([
        ...
        const custom_fields = [];
        for (const f of Array.from(res.data.api_user_fields.custom_fields)) {
    ...
    */
    deleteFieldById(id) {
      const promise = this.Api.sendDelete(`/user_fields/${id}`).then(() => this.removeListModelById(id));
      return promise;
    }
