...
define([
  ...
  let UserFields;
  return UserFields = (function() {
    UserFields = class UserFields extends BaseListEdit {
      static initClass() {
        this.$inject = ['Api', '$q'];
      }
      ...
      _doLoadList() {
        ...
        this.Api.sendDataGet([
          ...
          const custom_fields = [];
          for (let f of Array.from(res.data.api_user_fields.custom_fields)) {
      ...
      saveDisplayOrder(display_orders) {
        return this.Api.sendPostJson('/user_fields/display-order', {display_orders});
      }
      ...
      */
      deleteFieldById(id) {
        const promise = this.Api.sendDelete(`/user_fields/${id}`).then(() => {
      ...
      */
      loadEditFieldData(id) {
        ...
        if (id) {
          this.Api.sendGet(`/user_fields/${id}`).then( result => {
            const data = {};
      ...
      */
      saveFormModel(fieldModel, formModel) {
        ...
        if (fieldModel.id) {
          ...
          promise = this.Api.sendPutJson('/user_fields', postData).success( data => fieldModel.id = data.field_id);
        }
