  ...
  class Admin_Usersources_Ctrl_EditInstance extends Admin_Ctrl_Base {
    ...
    initialLoad() {
      ...
      this.listCtrl().refresh().then(() => {
        ...
        this.listCtrl().usersources.map((source) => {
          ...
          if (s.source_type === 'Application\\DeskPRO\\Usersource\\Adapter\\DeskPRO') { return; }
    ...
    saveSettings() {
      ...
      if (this.presaveCallback) {
        return this.presaveCallback().then(() => this.doSaveSettings().catch(() => this.stopSpinner('saving_settings', true))
        , () => this.stopSpinner('saving_settings', true));
      }
      return this.doSaveSettings().finally(() => this.stopSpinner('saving_settings', true));
    }
    ...
    doSaveSettings() {
      ...
      this.Api.sendPostJson(`/apps/instances/${this.instanceId}`, postData).then(() => {
        this.Api.sendGet(`/usersources/${this.usersourceType}/app-${this.instanceId}/extra-details`).then(result => this.$scope.usersource_details = result.data.usersource_details);
    ...
    startDelete($event) {
      ...
      const doDelete = () => this.Api.sendDelete(`/apps/instances/${this.app.id}`).success(() => {
