define([
  ...
  class Admin_EmailStatus_Ctrl_SourceList extends Admin_Ctrl_Base {
    ...
    initialLoad() {
      ...
      const p2 = this.Api.sendGet('/email_accounts').success(data => this.$scope.email_accounts = data.email_accounts);
      this.Api.sendGet('/email_status/stats').success(data => this.$scope.counts = {
    ...
    loadResults(fallbackPrevPage) {
      ...
      const promise = this.Api.sendGet('/email_status/sources', { filter: this.filter }).success((data) => {
        ...
        this.page_nums = [];
        for (let i = 1, end = this.num_pages, asc = end >= 1; asc ? i <= end : i >= end; asc ? i++ : i--) {
