define [
  ...
  class ReportCustomAbstract extends BaseListEdit
    ...
    _doLoadList: ->
      ...
      @Api.sendGet('/reports/' + @getUrlPart() + '/custom').success( (data) =>
    ...
    ###
    deleteReportById: (id) ->
      promise = @Api.sendDelete('/reports/' + @getUrlPart() + '/' + id).success( =>
    ...
    saveFormModel: (model, formModel, queryParts) ->
      ...
      if model.id
        promise = @Api.sendPostJson('/reports/' + @getUrlPart() + '/' + model.id, {report: postData, parts: queryParts})
      else
        promise = @Api.sendPutJson('/reports/' + @getUrlPart(), {report: postData, parts: queryParts}).success( (data) ->
          model.id = data.id
        )
