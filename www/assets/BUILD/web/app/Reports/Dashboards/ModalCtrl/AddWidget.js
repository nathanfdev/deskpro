define(['DeskPRO/Util/Arrays',], Arrays => [
  ...
  function($scope, $q, $modalInstance, DashboardsInfo, DashboardWidgetService, report_id, TemplateManager) {
    ...
    load_promises.push(DashboardWidgetService.getReports().then(function(result) {
      $scope.reports = result.reports;
      for (let label of Array.from(result.labels)) {
    ...
    $scope.filterByLabels = function() {
      if ($scope.selectedLabels !== 0) {
        return $scope.prefilteredReports = $scope.reports.filter(function(report) {
          for (let label of Array.from(report.labels)) { if ($scope.isActiveLabel(label)) { return true; } }
        });
      } else {
    ...
    $scope.filterBySearchText = function(value) {
      if ($scope.searchText === '') {
        ...
        if (value.title.toLocaleLowerCase().indexOf(search) >= 0) {
          ...
          for (let label of Array.from(value.labels)) { if (label.toLocaleLowerCase().indexOf(search) >= 0) { return true; } }
          return false;
        }
      }
    };
