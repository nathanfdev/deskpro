...
<div class="datetimepicker table-responsive" style="position: absolute; z-index: 10000; background: #fff; box-shadow: 0 0 15px rgba(0,0,0,0.7);">
    <table class="table table-striped">
      ...
      controller($scope) {
        ...
        var dataFactory = {
          ...
          month(unixDate) {
            ...
            for (let i = 0; i <= 11; i++) {
              ...
              result.dates.push(dateObject({
                ...
                active:  monthMoment.format(activeDate === 'YYYY-MMM')
              })
              );
            }
            return result;
          ...
          setTime(unixDate) {
            ...
            if ($scope.ngModel) {
              let format = 'll';
              if (($scope.options.minView === 'hour') || ($scope.options.minView === 'minute')) { format = 'lll'; }
      ...
      link($scope, $el, $attr) {
        ...
        return $scope.$root.$on('dp.datetime.show', ($event, ngModel, $callee, options) => {
