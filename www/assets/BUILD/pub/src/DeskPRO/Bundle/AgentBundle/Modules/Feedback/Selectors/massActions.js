...
export const massActionsSelector = createSelector(
  ...
  (navState, categories, types, labels) => {
    ...
    massActions
      .push({ label: 'Status', type: 'set_action', param: 'set_status', quickFilter: true, options: statusOptions });

    // Category options
    const categoryOptions = categories.toArray().map(cat => ({ label: cat.get('input'), value: cat.get('input') }));
    massActions.push({
      label:       'Category',
      type:        'set_action',
      param:       'set_category',
