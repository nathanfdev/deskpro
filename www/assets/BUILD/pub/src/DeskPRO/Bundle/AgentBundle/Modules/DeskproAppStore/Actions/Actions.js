...
export const DESKPRO_APPSTORE_LOAD_APP_STATE = 'DESKPRO_APPSTORE_LOAD_APP_STATE';
 ...
 * creates an action that will load the app configuration for the current security principal
 */
export const loadAllAppState = createAction(
  DESKPRO_APPSTORE_LOAD_APP_STATE,
  (appId, api, callback, eventBus) =>  api.sendGet(`DP_API/apps/${appId}/state`)
    .then(httpResponse => httpResponse.data)
    .then(state => { callback(state); return state; } )
    .then(state => eventBus.dispatch(DESKPRO_APPSTORE_LOAD_APP_STATE, appId, state))
);

/**
 * creates an action that will load the app configuration for the current security principal
 */
export const loadAOneAppState = createAction(
  DESKPRO_APPSTORE_LOAD_APPS,
  (appId, name, api) =>  api.sendGet(`DP_API/apps/${appId}/state/${name}`)
    .then(httpResponse => httpResponse.data)
);
