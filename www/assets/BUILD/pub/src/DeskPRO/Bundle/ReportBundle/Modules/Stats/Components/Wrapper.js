...
class Wrapper extends React.Component {
  ...
  static activateLabel(clickedLabel, labels, toggle = true) {
    ...
    let newActiveLabels = labels.reduce((reduced, value) => reduced + (value.get('active') ? 1 : 0), 0);
    if (entry) {
      const index = entry[0];
      const label = entry[1].set('active', !toggle ? true : !entry[1].get('active'));
      const newLabels = labels.set(index, label);
      newActiveLabels = newLabels.reduce((reduced, value) => reduced + (value.get('active') ? 1 : 0), 0);
