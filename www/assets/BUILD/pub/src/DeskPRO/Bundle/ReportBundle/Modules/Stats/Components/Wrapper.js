...
@connect(state => ({
  ...
  labels:         state.Application.reports.get('labels'),
}))
class Wrapper extends React.Component {
  ...
  static activateLabel(clickedLabel, labels, toggle = true) {
    const entry = labels.findEntry(value => value.label === clickedLabel);
    let newActiveLabels = labels.reduce((reduced, value) => reduced + (value.active ? 1 : 0), 0);
    if (entry) {
      const index = entry[0];
      const label = entry[1];
      label.active = !toggle ? true : !label.active;
      newActiveLabels = labels.reduce((reduced, value) => reduced + (value.active ? 1 : 0), 0);
      return { newLabels: labels.set(index, label), newActiveLabels, label: entry[1] };
    }
  ...
  constructor(props) {
    ...
    if (props.labels && props.labels.size > 0) {
      newLabels = Wrapper.transformLabels(props.labels);
    }
  ...
  onChangeFilterText(value) {
    ...
    let matches = Wrapper.regex.exec(value);
    let state = this.state;
    state.searchText = value;
    state.labels = labels.map((label) => { const newLabel = label; newLabel.active = false; return newLabel; });
    state.activeLabels = 0;

    while (matches !== null) {
      const result = Wrapper.activateLabel(matches[1], labels, false);
      labels = result.newLabels;
      activeLabels = result.newActiveLabels;
      matches = Wrapper.regex.exec(value);
  ...
  onLabelClick(clickedLabel) {
    const { newLabels, newActiveLabels } = Wrapper.activateLabel(clickedLabel, this.state.labels);
    let newSearchText = this.state.searchText.replace(Wrapper.regex, '');
    const labels = newLabels.filter(value => value.active).map(value => `[${value.label}]`).toList().toJS();
  ...
  setLabels(props) {
    if (props.labels && this.state.labels.size < 1) {
      const newLabels = Wrapper.transformLabels(props.labels);
  ...
  filter(value) {
    ...
    const actualSearch = searchText.replace(Wrapper.regex, '').trim().toLowerCase();
    ...
    let labelsToCheck = [];
    if (activeLabels > 0) {
      labelsToCheck = labels.filter(label => label.active).map(label => label.label).toList().toJS();
      result = result && value
        .get('labels')
  ...
  render() {
    const { customReports, builtInReports, reportsLoaded } = this.props;
    const { labels, activeLabels, searchText } = this.state;

    const filteredCustomReports = customReports.filter(this.filter);
    const filteredBuiltInReports = builtInReports.filter(this.filter);
