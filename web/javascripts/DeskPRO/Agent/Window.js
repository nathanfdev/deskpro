...
DeskPRO.Agent.Window = new Orb.Class({
  ...
  init: function () {
    ...
    this.util = {
      ...
      reloadInterface: function () {
        $('#reload_overlay').show().on('click', function (ev) {
          ev.stopPropagation();
        });
  ...
  initPage: function () {
    ...
    if (!window.DeskPRO_FragmentRouter) {
      ...
      window.DeskPRO_FragmentRouter = {
        ...
        getFragmentPattern: function () {
          return '';
        },
        getFragmentType: function () {
          return '';
        },
        getUrl: function () {
          return '';
        },
        getUrlNamedArgs: function () {
          return '';
        }
      };
    }
    ...
    var drag = function () {
      ...
      d.start = function ($c, direction) {
        ...
        d.timer = setInterval(function () {
          $c.scrollTo(direction + d.step + 'px');
        }, d.interval);
      };
      d.stop = function () {
    ...
    Array.each(segments, function (hash, i) {
      ...
      if (type == 'vis') {
        ...
        this.loadPage(url, {
          url_fragment: hash,
          noToggle: true,
          ignore_perm_error: startRestoreHash.replace(/\.o/, '').indexOf(hash.replace(/\.o/, '')) != -1
        });
      }
    }, this);
  ...
  _globalHandleAjaxError: function (event, xhr, ajaxOptions, errorThrown, force) {
    ...
    if (xhr && xhr.status && data && data.error && data.error == 'update_running') {
  ...
  _initRoutes: function () {
    ...
    this.addPageRouteLoader('ticket', (function (routeData) {
      ...
      routeData.tabLoad = function () {
        loaded[ticketId] = setTimeout(function () {
          delete loaded[ticketId]
        }, 400);
    ...
    $('#dp_nav [data-section-handler], #agent_chat_section').each(function () {
      ...
      if (++count) {
        handler.addEvent('sectionInit', function () {
          window.setTimeout(function () {
            handler._loadAutoLoadRoutes(true);
          }, secttimeout);
      ...
      if (!el.is('.no-click-switch')) {
        el.on('click', function () {
          self.switchToSection(el.attr('id'))
        });
      }
    });
  ...
  prepareWidgetedHtml: function (html) {
    ...
    $('style[data-widget]').each(function () {
      cssExists[$(this).data('widget')] = $(this).data('hash');
    });
