...
/**
 * A Twitter group editor positions rows in a popover overlay with rows
 * that match up to Twitter group titles on the left.
 */
DeskPRO.Agent.Widget.TwitterGroupEditor = new Orb.Class({
	...
	updatePositions: function() {
		...
		$(this.options.elements, listEl).each((function(i, el) {
			...
			var pos = el.position();
			var editEl = $('.filter-' + type, this.controlElement);
	...
	close: function() {
		...
		var values = {};
		this.controlRealEl.find('.filter-row').each(function() {
			values[$(this).data('type')] = $(this).find('.field-option').val();
		});

		this.fireEvent('close',[values, self]);
