(function() {
  define(function() {
    ...
    */
    ...
    var Admin_Main_Directive_DpStatusUpdate;
    Admin_Main_Directive_DpStatusUpdate = [
      'Api', 'Growl', function(Api, Growl) {
        return {
          restrict: 'AE',
          template: "<i class='spinner-xsmall' ng-show='update_in_progress'></i>{{status_update_message}}",
          link: function(scope, element, attrs) {
            var defaultMessage, statusUpdateUrl, updateCompletedGrowlMessage, updateImmediate, updateInterval, validStatuses,
              _this = this;
            statusUpdateUrl = attrs.dpStatusUpdate;
            updateInterval = attrs.statusUpdateInterval ? attrs.statusUpdateInterval : 5000;
            defaultMessage = attrs.statusUpdateDefaultMessage ? attrs.statusUpdateDefaultMessage : '';
            updateCompletedGrowlMessage = attrs.statusUpdateCompletedGrowlMessage ? attrs.statusUpdateCompletedGrowlMessage : null;
            updateImmediate = attrs.statusUpdateImmediate ? attrs.statusUpdateImmediate = attrs.statusUpdateImmediate === 'true' : true;
            scope.status_update_message = defaultMessage;
            scope.update_in_progress = false;
            validStatuses = ['pending', 'progress'];
            return scope.$watch(attrs.statusUpdateConditions, function(newVal, oldVal) {
              var doGetRequest, interval;
              scope.update_in_progress = true;
              doGetRequest = function() {
                var _this = this;
                return Api.sendGet(statusUpdateUrl).then(function(res) {
...
/*
//@ sourceMappingURL=DpStatusUpdate.js.map
*/