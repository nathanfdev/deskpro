  ...
  AgentApp.directive('dpTextOverflow', [function () {
	return {
	  ...
	  link: function (scope, element, attr) {
		...
		element.on('init.dptextoverflow', function () {
		  init();
		});
	  ...
	  link: function (scope, $el, attr) {
		...
		var updateSearch = function () {
		  ...
		  $http({
			...
			scope.resultGroups = scope.resultGroups.filter(function (v) {
			  return v.results && v.results.length;
			});
	...
	$scope.fetchPage = function (page) {
	  ...
	  $http({
		...
		data.data.each(function (item) {
		  $scope.$parent[$scope.listName].push(item);
		});
		$scope.page = page;
	  }, function () {
		$scope.isLoading = false;
