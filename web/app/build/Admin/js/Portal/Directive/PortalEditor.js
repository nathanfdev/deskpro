(function() {
  define(['DeskPRO/Util/Util', 'ColorPicker'], function(Util) {
    var DeskPRO_getPlainTpl, PortalEditor, createOverlayModal, editorAjaxClient, modalMarkup;
    DeskPRO_getPlainTpl = function(el) {
      var html;
      el = $(el);
    ...
    createOverlayModal = function(options) {
      ...
      if (options.contentElement) {
        ...
        if (options.onBeforeOverlayOpened) {
          options.onBeforeOverlayOpened({
            wrapperEl: wrapperEl,
            overlay: inst
          });
        }
      } else if (options.contentAjax) {
        editorAjaxClient(options.contentAjax).done(function(data) {
          applyToBody(data);
          if (options.onContentSet) {
            return options.onContentSet({
              wrapperEl: wrapperEl,
              overlay: inst
            });
          }
