...
class Working01 extends AbstractGenerator
{
	...
	protected function _genTicketsForUser(\DeskPRO\DBAL\Connection $db, $person_id, $num_tickets)
	{
		...
		while ($num_tickets-- > 0) {
			...
			} elseif ($ticket['status'] == 'open' AND $ticket['sub_status'] == 'awaiting_user') {
				$ticket['awaiting_user_at'] = $created_at->add(new \DateInterval('PT'.mt_rand(4000, 345600).'S'))->format('Y-m-d H:i:s');
			}

			$db->insert('tickets', $ticket);
			$ticket_id = $db->lastInsertId();

			$db->insert('ticket_subjects', array(
				'ticket_id' => $ticket_id,
				'subject' => $subject
			));
