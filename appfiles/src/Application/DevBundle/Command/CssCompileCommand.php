 ...
 * /static
 */
class CssCompileCommand extends \Symfony\Bundle\FrameworkBundle\Command\ContainerAwareCommand
{
	protected function configure()
	{
		$this->setDefinition(array(
			new InputArgument('pack', InputArgument::REQUIRED, 'The pack to compile. Example: agent.vendors'),
		))->setName('dpdev:css-compile');
	}
	...
	protected function execute(InputInterface $input, OutputInterface $output)
	{
		...
		$packs = App::getConfig($pack_name, null, 'css-sources');
		if (!$packs) {
		...
		foreach ($packs as $pack_name => $pack) {
			...
			$filepath = $static_path . 'build-css/' . $pack['out'];
			$filepath_tmp = $static_path . 'build-css/' . $pack['out'] . '.build';
