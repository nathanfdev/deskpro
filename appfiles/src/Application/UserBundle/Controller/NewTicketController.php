...
class NewTicketController extends AbstractController
{
	...
	public function handleLoginAction()
	{
		...
		if (!$result->isValid()) {
			$html = $this->renderView('UserBundle:NewTicket:user-email.html.twig', array('login_error' => true));
			return $this->createJsonResponse(array(
				'html' => $html,
			));
		}
		...
		$html = $this->renderView('UserBundle:NewTicket:user-email.html.twig', array('person' => $person));
