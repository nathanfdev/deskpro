...
class Upgrader
{
	...
	public function performUpgrade(\DateTime $version, OutputInterface $output)
	{
		...
		if (isset($up_status[1])) {
			...
			if (preg_match('#^(\d{14})\((\d+),(\d+)\)$#', $up_status, $up_status)) {
	...
	public function getUpgradeVersions()
	{
		if ($this->upgrade_versions === null) {
			$finder = new Finder();
			$finder->files()
				->in(ROOT.'/src/DeskPRO/Build/Upgrade')
				->name('/\\d{14}\.php/');
	...
	public function setCurrentVersion(\DateTime $version)
	{
		if (!@file_put_contents($this->getVersionFilePath, VersionReader::getVersionString($version))) {
			throw new \RuntimeException('Could not write version file');
		}
	}
