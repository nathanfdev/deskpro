...
class Upgrade20110808132103 extends UpgradeAbstract
{
	public function step1()
	{
		$this->output->writeln('Create ticket_snippets table');

		try {
			App::getDb()->exec("CREATE TABLE ticket_snippets (id INT AUTO_INCREMENT NOT NULL, person_id INT DEFAULT NULL, is_global TINYINT(1) NOT NULL, category VARCHAR(255) NOT NULL, title VARCHAR(255) NOT NULL, snippet LONGTEXT NOT NULL, INDEX IDX_6848095D217BBB47 (person_id), PRIMARY KEY(id)) ENGINE = InnoDB");
			App::getDb()->exec("CREATE TABLE ticket_snippet_to_team (team_id INT NOT NULL, INDEX IDX_7ECBD512296CD8AE (team_id), PRIMARY KEY(team_id)) ENGINE = InnoDB");
			App::getDb()->exec("ALTER TABLE ticket_snippets ADD FOREIGN KEY (person_id) REFERENCES people(id) ON DELETE SET NULL");
			App::getDb()->exec("ALTER TABLE ticket_snippet_to_team ADD FOREIGN KEY (team_id) REFERENCES agent_teams(id) ON DELETE CASCADE");
		} catch (\Exception $e) {
